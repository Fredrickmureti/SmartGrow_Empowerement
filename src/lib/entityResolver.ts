...
/**
 * Resolve a product by name or SKU (case-insensitive).
 * Auto-creates if not found.
 */
export class ProductResolver {
  ...
  constructor(
    ...
    if (!businessId) {
      throw new Error(
        "ProductResolver requires an active business_id (legal entity). " +
        "Products cannot be created without a company assignment."
      );
    }
    if (existingProducts) {
      for (const p of existingProducts) {
        this.cache.set(p.name.toLowerCase().trim(), p.id);
        if (p.sku) this.cache.set(`sku:${p.sku.toLowerCase().trim()}`, p.id);
      }
    }
  }

  async resolve(nameOrSku: string, unitPrice?: number): Promise<ResolvedEntity> {
    const key = nameOrSku.toLowerCase().trim();
    if (!key) throw new Error("Empty product name/SKU");

    // Check cache by name and SKU
    const cachedByName = this.cache.get(key);
    if (cachedByName) return { id: cachedByName, wasCreated: false };
    const cachedBySku = this.cache.get(`sku:${key}`);
    if (cachedBySku) return { id: cachedBySku, wasCreated: false };
    ...
    const { data: bySku } = await supabase
      ...
      .eq("organization_id", this.organizationId)
      .eq("business_id", this.businessId)
      .ilike("sku", nameOrSku.trim())
      .limit(1);

    if (bySku && bySku.length > 0) {
      this.cache.set(key, bySku[0].id);
      return { id: bySku[0].id, wasCreated: false };
    }
