/**
 * ProductDetailPanel — the single, context-aware Product Detail surface.
 *
 * Replaces the legacy ProductDetailDialog (Products page) and
 * ProductDetailDrawer (Inventory page). One drawer, one data source, tabs
 * rendered conditionally based on the product's actual configuration
 * (multi-UoM, lot tracking, expiry tracking, accounting links).
 *
 * Hook order is unconditional. Every `useX(...)` call happens before any
 * early return — this is the regression fix for the original
...
export function ProductDetailPanel({
  ...
  // ─── Hooks (UNCONDITIONAL — must run on every render) ───────────────────
  ...
  const isLowStock = trackInventory && reorderLevel > 0 && onHand > 0 && onHand <= reorderLevel;
  const isNegative = trackInventory && onHand < 0;

  // Tab registry — built from a stable list, filtered by configuration.
  // Every tab maps to one component; no hook is ever called conditionally.
  const tabs = useMemo(() => {
    const all = [
      { id: "overview", label: "Overview", show: true },
      { id: "stock", label: "Stock", show: trackInventory && !isService },
      { id: "lots", label: "Lots & Expiry", show: (isLotTracked || isExpiryTracked) && !isService },
      { id: "units", label: "Units & Packaging", show: hasMultiUoM && !isService },
      { id: "valuation", label: "Valuation", show: trackInventory && hasCost && !isService },
  ...
  }, [trackInventory, isService, isLotTracked, isExpiryTracked, hasMultiUoM, hasCost, hasAccounts]);

  // Replenishment signal — feeds the intelligence strip chip below the tiles.
  const signal = useMemo(
    ...
    [trackInventory, onHand, reserved, data?.incomingPo.totalQty, data?.velocityPerWeek],
  );
      ...
      <SheetContent className="w-full sm:max-w-2xl overflow-y-auto">
        <SheetHeader>
          <div className="flex items-start gap-3">
            <div className="h-12 w-12 rounded-md border bg-muted overflow-hidden flex items-center justify-center shrink-0">
              {product?.image_url ? (
                <img src={product.image_url} alt={headlineName} className="h-full w-full object-cover" />
              ) : (
            ...
            <div className="flex-1 min-w-0">
              ...
              <SheetDescription className="flex flex-wrap items-center gap-2 mt-1">
                {headlineSku && <span className="text-xs">SKU: {headlineSku}</span>}
                {product && (
                  <>
                    <Badge variant="outline" className="capitalize text-xs">{product.type}</Badge>
                    {product.is_active === false && <Badge variant="secondary" className="text-xs">Inactive</Badge>}
                    {isOutOfStock && <Badge variant="destructive" className="text-xs">Out of stock</Badge>}
                    {isLowStock && (
                      <Badge className="bg-warning/15 text-warning border border-warning/30 text-xs">Low stock</Badge>
                    )}
                    {isNegative && <Badge variant="destructive" className="text-xs">Negative stock</Badge>}
                    {isLotTracked && <Badge variant="outline" className="text-xs">Lot-tracked</Badge>}
                    {isExpiryTracked && <Badge variant="outline" className="text-xs">Expiry</Badge>}
                    {hasMultiUoM && <Badge variant="outline" className="text-xs">Multi-UoM</Badge>}
                  </>
                )}
              </SheetDescription>
            </div>
        ...
        {isLoading && !product ? (
          ...
          <div className="mt-4 space-y-4">
              ...
              return (
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  <StatCard
                    ...
                    value={fmt(onHand)}
                    sub={packs.length > 0 && onHand > 0 ? `${Number(onHand.toFixed(3))} ${baseLabel}` : undefined}
                    tone={isNegative ? "destructive" : isOutOfStock ? "destructive" : isLowStock ? "warning" : "default"}
                  />
                  <StatCard label="Reserved" value={fmt(reserved)} />
                  <StatCard label="Available" value={fmt(available)} tone={available < 0 ? "destructive" : "default"} />
                  <StatCard
                    label="Value"
                    value={formatCurrency(onHand * Number(product.cost_price ?? 0))}
                  />
                </div>
              );
            })()}

            {(data.incomingPo.totalQty > 0 || data.velocityPerWeek > 0 || signal.tier !== "untracked") && (
              <div className="flex flex-wrap gap-2 text-xs">
                {trackInventory && !isService && signal.tier !== "no-velocity" && signal.tier !== "untracked" && (
                  <Badge
                    variant="outline"
                    title={signal.hint}
                    className={
                ...
                {data.incomingPo.totalQty > 0 && (
                  <Badge variant="outline" className="font-normal">
                    {data.incomingPo.totalQty} incoming · {data.incomingPo.openOrders} PO{data.incomingPo.openOrders === 1 ? "" : "s"}
                  </Badge>
                )}
            ...
            <div className="flex flex-wrap gap-2">
              {onEdit && (
                <Button size="sm" variant="outline" onClick={() => { onOpenChange(false); onEdit(product.id); }}>
              ...
              {trackInventory && !isService && (
                <>
                  ...
                  <Button
                    ...
                    className={isLowStock || isOutOfStock ? "border-warning/40 text-warning" : ""}
                  >
                    <Link to={`/purchases-app/purchase-orders?action=new&product=${product.id}`}>
                      <ShoppingCart className="h-3.5 w-3.5 mr-1" /> {isLowStock || isOutOfStock ? "Replenish" : "Create PO"}
                    </Link>
                  </Button>
                  <Button size="sm" variant="outline" asChild onClick={() => onOpenChange(false)}>
                  ...
                  <Button size="sm" variant="outline" asChild onClick={() => onOpenChange(false)}>
                    <Link to={`/inventory-app/stock?tab=movements&product=${product.id}`}>
              ...
              {onDelete && (
                <Button
                  ...
                  onClick={() => { onOpenChange(false); onDelete(product.id); }}
                >
            ...
            <Tabs defaultValue={tabs[0]?.id ?? "overview"}>
              ...
              <TabsContent value="activity">
                <div className="pt-2">
                  <DocumentHistoryTab entityType="product" entityId={product.id} />
                </div>
              </TabsContent>
            </Tabs>
          </div>
...
function StatCard({
  ...
  return (
    <div className="rounded-md border p-2.5">
      ...
      <p className={`text-base font-semibold leading-tight ${toneClass}`}>{value}</p>
