...
import { CustomerStatementData } from "@/hooks/useCustomerStatements";
...
export function StatementPreview({ data }: StatementPreviewProps) {
  ...
  // Branding MUST come from the issuing business + branch — not the
  // user's currently-selected scope. Defensive `?? null` handles
  // legacy/test data missing the new fields.
  ...
  const renderReference = (txn: CustomerStatementData['transactions'][number]) => {
  ...
  return (
    <div className="bg-background border rounded-lg shadow-sm print:shadow-none print:border-0 min-w-0">
        ...
        <div className="flex items-start gap-3 sm:gap-4 min-w-0">
          {branding?.logo_url && (
            <img
              ...
              alt={branding.name}
              className="h-10 w-10 sm:h-14 sm:w-14 rounded object-contain shrink-0"
            />
          )}
          <div className="min-w-0">
            ...
            {branding?.phone && <p className="text-xs sm:text-sm text-muted-foreground">{branding.phone}</p>}
            {branding?.email && <p className="text-xs sm:text-sm text-muted-foreground truncate">{branding.email}</p>}
          </div>
        </div>
        <div className="text-left sm:text-right shrink-0">
          <h1 className="text-lg sm:text-2xl font-bold tracking-tight text-primary uppercase">Statement</h1>
        ...
        <div className="min-w-0">
          ...
          {data.contact.tax_id && (
            <p className="text-xs sm:text-sm text-muted-foreground font-mono truncate">
              Customer ID / Tax ID: {data.contact.tax_id}
            </p>
          )}
      ...
      {/* Transaction Detail — Mobile cards + Desktop table */}
                    ...
                    <TableCell>
                      <Badge
                        ...
                        className={`capitalize text-xs ${
                          txn.type === "invoice" ? "border-blue-300 text-blue-700 dark:text-blue-400" :
                          txn.type === "payment" ? "border-green-300 text-green-700 dark:text-green-400" :
                  ...
                  <div className="flex gap-3">
                    {txn.debit > 0 && (
                      <span className="text-xs">
                        ...
                        <span className="font-medium">{formatCurrency(txn.debit)}</span>
                      </span>
                    )}
                    {txn.credit > 0 && (
                      <span className="text-xs">
                        <span className="text-muted-foreground">Credit: </span>
                        <span className="font-medium">{formatCurrency(txn.credit)}</span>
                      </span>
                    )}
                  </div>
      ...
      {/* Aging Summary */}
      <div className="p-4 sm:px-6 sm:py-4">
        <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2 sm:mb-3">Aging Summary</p>
        {/* Desktop aging table */}
      ...
      <div className="px-4 sm:px-6 py-4 sm:py-5 bg-muted/30 border-t flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3">
        <div className="text-xs sm:text-sm text-muted-foreground">
          <p>Please remit payment by the due date shown on each invoice.</p>
          <p>Questions? Contact us at {branding?.email || branding?.phone || "our office"}.</p>
        </div>
        <div className="sm:text-right shrink-0">
          <p className="text-xs text-muted-foreground uppercase tracking-wider">Total Amount Due</p>
