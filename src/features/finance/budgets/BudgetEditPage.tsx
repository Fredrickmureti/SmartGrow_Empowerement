...
export default function BudgetEditPage() {
   ...
   * actuals in it. The config itself lives in `budgetScheduleExport` so the
   * budget list row menu produces the exact same server-build request.
   */
  const getScheduleExportConfig = useCallback(
    (): ExportConfig => buildBudgetScheduleExportConfig(budget),
    [budget],
  );
