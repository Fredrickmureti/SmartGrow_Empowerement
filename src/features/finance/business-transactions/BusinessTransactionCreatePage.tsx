...
export default function BusinessTransactionCreatePage() {
  ...
  return (
    ...
    >
      <Section title="Transaction" description="Guided form — no debit/credit knowledge needed">
        <FieldGrid>
          <FieldCell label="Transaction Type" span={2}>
            <Select
              value={type}
              onValueChange={(v) => {
      ...
      {(type === "owner_investment" || type === "owner_drawing") && (
        <Section title="Accounts">
          <FieldGrid>
            <FieldCell label="Bank / Cash Account" span={2}>
              <AccountCombobox
                accounts={accounts}
                value={bankAccountId}
            ...
            <FieldCell
              label={type === "owner_investment" ? "Equity Account" : "Owner's Drawing / Equity Account"}
              hint="Only Equity accounts are allowed here — owner contributions and drawings affect equity, not assets."
              span={2}
            >
              <AccountCombobox
                accounts={accounts}
      ...
      {type === "bank_transfer" && (
        <Section title="Accounts">
          <FieldGrid>
            <FieldCell label="Transfer From (Source)">
              <AccountCombobox
                accounts={accounts}
            ...
            <FieldCell label="Transfer To (Destination)">
              <AccountCombobox
                accounts={accounts}
      ...
      {(type === "loan_received" || type === "loan_payment") && (
        <Section title="Accounts">
          <FieldGrid>
            <FieldCell label="Bank / Cash Account">
              <AccountCombobox
                accounts={accounts}
                value={bankAccountId}
                onValueChange={setBankAccountId}
            ...
            <FieldCell label="Loan Account (Liability)">
              <AccountCombobox
                accounts={accounts}
                value={loanAccountId}
            ...
            {type === "loan_payment" && (
              <>
                <FieldCell label="Interest Portion (optional)">
                  <Input
                    type="number"
                    min="0"
                    step="0.01"
                ...
                {interestAmount && parseFloat(interestAmount) > 0 && (
                  <FieldCell label="Interest Expense Account">
                    <AccountCombobox
                      accounts={accounts}
      ...
      <Section title="Memo">
        <FieldGrid>
          <FieldCell label="Memo (optional)" span={2}>
            <Textarea
              value={memo}
