...
function ViewerInner() {
  ...
  const getExportConfig = useCallback((): ExportConfig => {
    ...
    // Server-build hints. Every export (PDF, CSV, XLSX, preview, email)
    // is regenerated by `render-report` from the same reportType + period
    // the table used, so an export can neither be empty nor be truncated
    // to the rows the browser happens to hold. `rows`/`columns` remain on
    // the config only as the fallback for surfaces without a registry key.
    const cfg: ServerBuildConfig = {
