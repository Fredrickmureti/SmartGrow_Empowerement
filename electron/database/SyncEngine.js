...
class SyncEngine {
    ...
    async fullSync() {
        ...
        try {
            ...
            // Log sync completion
            await AuditLogger_1.auditLogger.logSync('all', result.recordsPulled + result.recordsPushed, 'up');
        }
        catch (error) {
    ...
    async pullTable(config) {
        ...
        try {
            ...
            const meta = metaStmt.get(config.tableName);
            const lastSync = meta?.last_server_timestamp || '1970-01-01T00:00:00Z';
            // Fetch updated records from Supabase
            const response = await this.supabaseRequest(`${config.supabaseTable}?organization_id=eq.${this.organizationId}&${config.timestampField}=gt.${lastSync}&order=${config.timestampField}.asc`, 'GET');
            if (!response.ok) {
        ...
        `);
                updateMeta.run(config.tableName, new Date().toISOString(), lastRecord[config.timestampField], recordsProcessed, 'down');
            });
            transaction();
        }
        catch (error) {
    ...
    async pushTable(config) {
        ...
        LIMIT 100
      `);
            const pendingRecords = pendingStmt.all();
            for (const record of pendingRecords) {
                try {
                    ...
                    if (record.server_id) {
                        ...
                        response = await this.supabaseRequest(`${config.supabaseTable}?id=eq.${record.server_id}`, 'PATCH', data);
                    }
                    else {
                        // Insert new record
                        response = await this.supabaseRequest(config.supabaseTable, 'POST', data);
                    }
                    if (response.ok) {
            ...
            `).run(serverId || record.server_id, new Date().toISOString(), record.id);
                        ...
                        // Conflict detected
                        const serverRecord = (await this.fetchServerRecord(config.supabaseTable, record.server_id));
                        conflicts.push({
                            table: config.tableName,
                            recordId: record.id,
     ...
     */
    async fetchServerRecord(table, id) {
        try {
     ...
     */
    async supabaseRequest(path, method, body) {
     ...
     * Resolve a conflict
     */
    resolveConflict(table, recordId, resolution) {
        const db = DatabaseManager_1.databaseManager.getDatabase();
