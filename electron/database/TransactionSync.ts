...
class TransactionSyncHandler {
   ...
   */
  saveTransactionLocally(data: TransactionSyncData): TransactionSyncResult {
    ...
    try {
      const transaction = db.transaction(() => {
        ...
        if (data.transaction.shift_id) {
          ...
          `).run(
            ...
            mpesaAmount,
            data.transaction.total,
            cashAmount - data.payments.reduce((sum, p) => sum + (p.change_given || 0), 0),
            data.transaction.shift_id
          );
        }
      });
      ...
      // Log the transaction
      auditLogger.logTransaction(data.transaction.id, data);
   ...
   */
  async syncTransactionToServer(transactionId: string): Promise<TransactionSyncResult> {
    const db = databaseManager.getDatabase();

    try {
      // Get transaction with items and payments
      const transaction = db.prepare('SELECT * FROM pos_transactions WHERE id = ?').get(transactionId) as any;
      if (!transaction) {
        return { success: false, transactionId, error: 'Transaction not found' };
      }

      const items = db.prepare('SELECT * FROM pos_transaction_items WHERE transaction_id = ?').all(transactionId) as any[];
      const payments = db.prepare('SELECT * FROM pos_transaction_payments WHERE transaction_id = ?').all(transactionId) as any[];
   ...
   */
  voidTransaction(transactionId: string, reason: string, voidedBy: string): boolean {
    const db = databaseManager.getDatabase();

    try {
      const transaction = db.transaction(() => {
        // Get original transaction
        const original = db.prepare('SELECT * FROM pos_transactions WHERE id = ?').get(transactionId) as any;
        if (!original) {
      ...
      // Log the void
      auditLogger.logVoid(transactionId, reason);
...
// Singleton instance
export const transactionSync = new TransactionSyncHandler();
