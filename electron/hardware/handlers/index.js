...
exports.HARDWARE_HANDLERS = void 0;
exports.setPaymentService = setPaymentService;
...
function setPaymentService(svc) {
    _paymentService = svc;
}
function getPaymentService() { return _paymentService; }
/**
...
function driverFor(a) {
    ...
    if (cached)
        return cached;
    const d = (0, drivers_1.buildDriver)(a);
    if (d)
        driverCache.set(a, d);
    return d;
}
/** Build a handler that delegates to the driver's `handle(cmd)`. */
function makeOpHandler(a, op) {
    return async (cmd) => {
...
const paymentOp = (op) => (_a) => async (cmd) => {
...
exports.HARDWARE_HANDLERS = {
    'receipt_printer:print_receipt': (a) => makeOpHandler(a, 'print_receipt'),
    'kitchen_printer:print_ticket': (a) => makeOpHandler(a, 'print_ticket'),
