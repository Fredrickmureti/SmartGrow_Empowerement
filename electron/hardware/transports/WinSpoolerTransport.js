...
exports.WinSpoolerTransport = void 0;
exports.__setWinSpoolRunner = __setWinSpoolRunner;
const node_child_process_1 = require("node:child_process");
const node_os_1 = require("node:os");
const promises_1 = require("node:fs/promises");
const node_path_1 = require("node:path");
const node_crypto_1 = require("node:crypto");
async function defaultPwsh(script, timeoutMs) {
    return new Promise((resolve) => {
        ...
        const child = (0, node_child_process_1.spawn)(exe, ['-NoProfile', '-NonInteractive', '-Command', script], {
...
exports.WinSpoolerTransport = {
    ...
    async send(target, bytes) {
        ...
        const tmp = (0, node_path_1.join)((0, node_os_1.tmpdir)(), `pos-spool-${(0, node_crypto_1.randomUUID)()}.prn`);
        try {
...
`.trim();
    ...
    async test(target) {
        ...
        const res = await _pwsh(script, 3000);
        if (res.code === 0)
            return { ok: true };
        return { ok: false, error: (res.stderr || res.stdout || `pwsh exited ${res.code}`).trim() };
    },
    isAvailable() { return process.platform === 'win32'; },
    async disconnect(_t) { },
};
//# sourceMappingURL=WinSpoolerTransport.js.map