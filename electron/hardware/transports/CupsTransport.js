...
exports.CupsTransport = void 0;
exports.__setCupsRunner = __setCupsRunner;
const node_child_process_1 = require("node:child_process");
async function defaultRunner(cmd, args, stdin, timeoutMs) {
...
/** Test seam. */
function __setCupsRunner(r) {
    _runner = r ?? defaultRunner;
}
exports.CupsTransport = {
    async list() {
        if (process.platform !== 'linux' && process.platform !== 'darwin')
            return [];
        const res = await _runner('lpstat', ['-p', '-d'], Buffer.alloc(0), 5000);
        if (res.code !== 0)
            return [];
        const queues = [];
    ...
    async send(target, bytes) {
        ...
        if (!target.queue)
            return { ok: false, error: 'cups target missing queue name' };
    ...
    async test(target) {
        if (process.platform === 'win32')
            return { ok: false, error: 'cups unavailable on win32' };
        const res = await _runner('lpstat', ['-p', target.queue], Buffer.alloc(0), 3000);
        if (res.code === 0 && /enabled|idle/i.test(res.stdout))
            return { ok: true };
...
//# sourceMappingURL=CupsTransport.js.map