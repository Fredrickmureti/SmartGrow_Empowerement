...
| Product | View | `ProductDetailPanel` peek on shared `DetailSheet` primitive (migrated 2026-07-14 from raw `<Sheet>`; badges rendered in body to keep `SheetDescription` `<p>` valid; data hook split into critical + deferred phases; quick actions hoisted to sticky `FooterActionBar` anchor="sheet" so Adjust / Transfer / Forecast / Replenish stay reachable on mobile). Note: aligns with sibling inventory peek `AdjustmentPeekSheet` — `PeekScaffold` is Sales-document-shaped (detail fields + line items + totals) and does not fit the Product tabs/stat-card layout. | Peek on `DetailSheet` | Done |
