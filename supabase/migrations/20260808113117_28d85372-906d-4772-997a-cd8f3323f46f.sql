...
BEGIN
  ...
  IF FOUND THEN
    ...
    IF v_rule.embedded_value_start IS NOT NULL AND v_rule.embedded_value_length IS NOT NULL THEN
      ...
      IF v_rule.kind = 'weighted_price' THEN
        v_price := v_embedded;
      ELSIF v_rule.kind = 'weighted_qty' THEN
        v_weight := v_embedded;
      END IF;
    END IF;

    SELECT * INTO v_id
    FROM public.resolve_product_identity(p_business_id, v_item_code, p_branch_id, false)
    LIMIT 1;
    v_have_id := FOUND;
  END IF;

  -- Do not combine this guard with a read from v_id. PostgreSQL may evaluate
  -- both sides of a boolean expression, and an unassigned record raises 55000.
  IF NOT v_have_id THEN
    SELECT * INTO v_id
    FROM public.resolve_product_identity(p_business_id, v_code, p_branch_id, false)
    LIMIT 1;
    v_have_id := FOUND;
  ELSIF coalesce(v_id.status, 'not_found') <> 'resolved' THEN
    SELECT * INTO v_id
    FROM public.resolve_product_identity(p_business_id, v_code, p_branch_id, false)
    LIMIT 1;
    v_have_id := FOUND;
  ...
  IF coalesce(v_id.status, 'not_found') = 'resolved' THEN
    IF v_item_code IS NULL OR v_id.matched_code::text <> v_item_code THEN
      v_rule_kind := NULL;
      v_weight := NULL;
      v_price := NULL;
    END IF;
...
REVOKE ALL ON FUNCTION public.pos_resolve_scan(uuid, uuid, text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.pos_resolve_scan(uuid, uuid, text) TO authenticated, service_role;