...
BEGIN
  ...
  -- Thermal-native kinds are the only ones that may legitimately resolve
  -- to an ESC/POS printer. Every other kind is an A4 paper document; a
  -- policy row pointing it at an ESC/POS thermal printer is a
  -- misconfiguration and would crash the rendering engine (which cannot
  -- fit an A4 invoice onto an 80mm receipt). Coerce those rows to PDF
  -- paper printing so the paper printer still receives the job.
  v_is_thermal_kind := v_short_type IN ('pos_receipt', 'kitchen_ticket');
  ...
  IF p_organization_id IS NOT NULL THEN
    ...
    IF v_business_id IS NOT NULL THEN
      ...
      IF FOUND THEN
        -- Base medium from the policy row.
        ...
        -- Invariant: only thermal-native kinds may go through ESC/POS.
        -- For everything else, coerce to PDF paper printing and normalise
        -- the paper size so the A4 renderer isn't asked to fit onto 80mm.
        IF v_policy_medium = 'escpos' AND NOT v_is_thermal_kind THEN
          v_policy_medium := 'pdf';
        ...
        THEN
          v_policy_target := jsonb_build_object(
            ...
            'params',        jsonb_build_object(
              ...
              'coerced_to_pdf', (v_policy_medium = 'pdf' AND v_policy.render_mode = 'escpos' AND NOT v_is_thermal_kind)
            )
          );
