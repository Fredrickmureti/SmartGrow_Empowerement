...
BEGIN
  ...
  -- Publish domain event to the shared outbox. Idempotency key = recall id
  -- so a retry with the same recall (should never happen — recall_id is
  -- freshly generated) writes at most one row.
  INSERT INTO public.business_event_outbox (
    org_id, event_type, source_doc_type, source_doc_id,
    payload, idempotency_key, actor_user_id, source
  ) VALUES (
    v_org_id,
    'product.recall.opened',
    'product_recall',
    v_recall_id,
    jsonb_build_object(
    ...
    'product.recall.opened:' || v_recall_id::text,
    v_user_id,
    'recall_lot'
  )
