...
RETURNS TABLE(
  ...
  opening_balance numeric, line_id uuid, journal_entry_id uuid, entry_date date,
  entry_number text, je_description text, line_description text, reference text,
  debit numeric, credit numeric, source_type text, source_id uuid,
  contact_name text, entry_status text, is_reversal boolean,
  reversal_of_number text, journal_book text, branch_name text,
  entry_currency text, original_debit numeric, original_credit numeric,
...
GRANT EXECUTE ON FUNCTION public.get_general_ledger(uuid, date, date, uuid, uuid[], boolean, uuid) TO authenticated;