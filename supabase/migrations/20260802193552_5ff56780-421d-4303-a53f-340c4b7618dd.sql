...
BEGIN
  ...
  CASE p_rpc
    ...
    -- Receiving audit Phase 4b: `receive_goods_to_wms` is NOT replay-whitelisted.
    -- Staging happens only inside `wms_post_receiving_session`; no client device
    -- may stage a goods receipt after the fact.

    -- Receiving audit Phase 6b: the mobile receiving loop captures lines
    -- through the offline queue, so the capture RPC must be replayable. The
    -- RPC is itself idempotent on (device_id, client_scan_id).
...
-- Receiving audit Step 4: only two document kinds are reachable from the UI
-- and supported by posting. Pin it in the schema.
UPDATE public.wms_receiving_sessions
   SET source_doc_type = NULL
 WHERE source_doc_type IS NOT NULL
   AND source_doc_type NOT IN ('purchase_order', 'inbound_shipment');

ALTER TABLE public.wms_receiving_sessions
  DROP CONSTRAINT IF EXISTS wms_receiving_sessions_source_doc_type_chk;

ALTER TABLE public.wms_receiving_sessions
  ADD CONSTRAINT wms_receiving_sessions_source_doc_type_chk
  CHECK (source_doc_type IS NULL OR source_doc_type IN ('purchase_order', 'inbound_shipment'));