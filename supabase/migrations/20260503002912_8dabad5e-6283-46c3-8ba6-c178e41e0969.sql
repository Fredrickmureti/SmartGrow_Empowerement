-- Branch-aware product+stock listing for sales forms.
-- Returns one row per product with on_hand/reserved/available aggregated from
-- warehouse_stock for the requested branch (or whole business when null).
...
AS $$
BEGIN
  -- Authorization: caller must have access to the business.
  ...
  -- If branch is specified, validate it belongs to the business.
  IF p_branch_id IS NOT NULL THEN
    IF NOT EXISTS (
      SELECT 1 FROM public.branches b
      WHERE b.id = p_branch_id AND b.business_id = p_business_id
    ) THEN
      RAISE EXCEPTION 'branch % does not belong to business %', p_branch_id, p_business_id
        USING ERRCODE = '22023';
    END IF;
  END IF;
  ...
  SELECT
    ...
    CASE WHEN p_branch_id IS NULL
         THEN 'All branches'
         ELSE COALESCE((SELECT name FROM public.branches WHERE id = p_branch_id), 'Branch')
    END AS branch_scope_label
...
GRANT EXECUTE ON FUNCTION public.list_products_with_branch_stock(uuid, uuid, uuid) TO authenticated;
