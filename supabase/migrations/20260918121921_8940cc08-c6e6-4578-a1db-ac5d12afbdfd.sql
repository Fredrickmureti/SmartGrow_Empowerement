CREATE OR REPLACE FUNCTION public.fa_create_asset(_business_id uuid, _asset jsonb, _payment_method text DEFAULT 'bank'::text, _settlement_account_id uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- A named settlement account (the specific bank / cash / till account the
  -- money actually left) wins over the generic mapping, so the bank statement
  -- and the ledger agree. An unpaid purchase always uses the payable control
  -- account and never a settlement account.
...
-- The 3-argument form is superseded by the 4-argument one above (same defaults),
-- so remove the ambiguous overload.
DROP FUNCTION IF EXISTS public.fa_create_asset(uuid, jsonb, text);