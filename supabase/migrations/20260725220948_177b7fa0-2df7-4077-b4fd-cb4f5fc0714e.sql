-- ADR 0091 §11 — audit parity: a payroll-sourced instalment must carry the
-- same detail on its lifecycle event / outbox row as a manual one.
...
BEGIN
  ...
  -- Audit parity: amount + source references travel with the event and the
  -- business_event_outbox row, identically for payroll and manual repayments.
