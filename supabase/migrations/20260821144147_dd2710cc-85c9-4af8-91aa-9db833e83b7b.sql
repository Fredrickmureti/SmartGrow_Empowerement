-- Phase A (ADR 0136): remove the COALESCE(<rate>, 1) parity fallback from the
-- historical leg of realized FX in the two multi-document settlement RPCs.

CREATE OR REPLACE FUNCTION public.record_multi_invoice_payment(_org_id uuid, _business_id uuid, _contact_id uuid, _allocations jsonb, _total_amount numeric, _payment_date date, _payment_method text DEFAULT 'bank_transfer'::text, _reference text DEFAULT NULL::text, _notes text DEFAULT NULL::text, _receipt_number text DEFAULT NULL::text, _created_by uuid DEFAULT NULL::uuid, _deposit_account_id uuid DEFAULT NULL::uuid, _receivable_account_id uuid DEFAULT NULL::uuid, _customer_credit_account_id uuid DEFAULT NULL::uuid, _branch_id uuid DEFAULT NULL::uuid, _exchange_rate numeric DEFAULT NULL::numeric, _request_id text DEFAULT NULL::text)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF _exchange_rate IS NOT NULL THEN
  ...
  LOOP
    ...
    -- No parity fallback (ADR 0136): the stamped booking rate is read as-is.
    SELECT id, invoice_number, total, COALESCE(amount_paid, 0) AS amount_paid,
           status, branch_id, journal_entry_id, exchange_rate
    ...
    -- A base-currency invoice is 1:1 by definition. A foreign invoice MUST carry
    -- the rate it was booked at; relieving it at parity would fabricate the
    -- realized FX difference.
    IF v_currency IS NULL OR v_currency = v_base_currency THEN
      v_book_rate := 1;
    ELSE
      v_book_rate := v_invoice.exchange_rate;
      IF v_book_rate IS NULL OR v_book_rate <= 0 THEN
        RAISE EXCEPTION 'Invoice % is denominated in % but carries no booking exchange rate. It cannot be settled until the document rate is stamped.',
          v_invoice.invoice_number, v_currency USING ERRCODE = '23514';
      END IF;
    END IF;
    ...
    v_ar_base := v_ar_base + ROUND(v_alloc.amount * v_book_rate, 2);
  ...
  v_je_id := post_journal_entry_atomic(
    ...
    false, false,
    v_lines, v_currency, v_settle_rate, NULL, _branch_id, false, false
  );
...
CREATE OR REPLACE FUNCTION public.record_multi_bill_payment(_org_id uuid, _business_id uuid, _vendor_id uuid, _allocations jsonb, _total_amount numeric, _payment_date date, _payment_method text DEFAULT 'bank_transfer'::text, _reference text DEFAULT NULL::text, _notes text DEFAULT NULL::text, _created_by uuid DEFAULT NULL::uuid, _bank_account_id uuid DEFAULT NULL::uuid, _payable_account_id uuid DEFAULT NULL::uuid, _branch_id uuid DEFAULT NULL::uuid, _request_id text DEFAULT NULL::text, _wht_rate numeric DEFAULT NULL::numeric, _wht_account_id uuid DEFAULT NULL::uuid, _credit_account_id uuid DEFAULT NULL::uuid, _exchange_rate numeric DEFAULT NULL::numeric)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  -- Settlement rate: resolved from the canonical rate book. Never invented.
  IF v_currency IS NULL OR v_currency = v_base_currency THEN
    v_settle_rate := 1;
  ELSE
  ...
  LOOP
    ...
    -- A base-currency bill is 1:1 by definition. A foreign bill MUST carry the
    -- rate it was booked at; relieving it at parity would fabricate the realized
    -- FX difference.
    IF v_currency IS NULL OR v_currency = v_base_currency THEN
      v_book_rate := 1;
    ELSE
      v_book_rate := v_bill.currency_rate;
      IF v_book_rate IS NULL OR v_book_rate <= 0 THEN
        RAISE EXCEPTION 'Bill % is denominated in % but carries no booking exchange rate. It cannot be settled until the document rate is stamped.',
          v_bill.bill_number, v_currency USING ERRCODE = '23514';
      END IF;
    END IF;
    ...
    v_ap_base := v_ap_base + ROUND(v_alloc.amount * v_book_rate, 2);
  ...
  v_je_id := public.post_journal_entry_atomic(
    ...
    v_lines,
    v_currency, v_settle_rate, NULL, _branch_id, false, false
  );
