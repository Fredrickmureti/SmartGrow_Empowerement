...
-- =====================================================
-- Customer Overpayment Correction & Credit Application Fix
-- =====================================================

-- 1. GUARDRAIL: Add overpayment safety check to record_payment_atomic
--    If overpayment occurs but no customer credit account is configured, RAISE ERROR
...
-- 2. NEW RPC: apply_credit_to_invoice_atomic
--    Atomically applies a credit note to an invoice with proper GL posting
...
BEGIN
  ...
  -- 1. Insert credit_note_applications record
  ...
  -- 4. Create balanced journal entry for GL
  --    Dr Customer Deposits & Advances (liability reduction)
  --    Cr Accounts Receivable (AR reduction)
  IF _customer_deposits_account_id IS NOT NULL AND _receivable_account_id IS NOT NULL THEN
    ...
    -- Dr Customer Deposits & Advances (reduce liability)
    INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit, credit, description)
