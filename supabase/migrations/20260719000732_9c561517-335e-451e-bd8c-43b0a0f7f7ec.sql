...
BEGIN
  ...
  -- Materialise tenders into the payments jsonb array expected by
  -- process_pos_transaction. Card metadata + mpesa receipt live in
  -- driver_payload on the tender row and get promoted to top-level
  -- fields here so _pos_record_payment inserts them onto the row.
