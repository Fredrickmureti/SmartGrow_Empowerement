-- ============================================================
-- ADR 0105 · Phase 8 — decommission the legacy carton catalogue
-- ============================================================

-- 1. Stop stamping the legacy column from the sanctioned RPC.
CREATE OR REPLACE FUNCTION public.assign_packaging_to_pack(p_carton_id uuid, p_packaging_type_id uuid)
 RETURNS wms_pack_cartons
 LANGUAGE plpgsql
 SECURITY DEFINER
...
-- 2. Remove the legacy cases from the offline replay dispatcher.
...
BEGIN
  ...
  CASE p_rpc
    ...
    -- Phase 5 (ADR 0105 §7): packaging identity on handling units.
    ...
    -- ADR 0105 Phase 8: legacy 'suggest_carton' / 'assign_carton_to_pack'
    -- cases removed; the packaging master is the only cartonisation path.
...
-- 3. Drop the legacy cartonisation functions.
DROP FUNCTION IF EXISTS public.suggest_carton(uuid, uuid[], numeric[]);
DROP FUNCTION IF EXISTS public.assign_carton_to_pack(uuid, uuid);

-- 4. Drop the legacy column (empty: verified 0 rows with a non-null value).
ALTER TABLE public.wms_pack_cartons DROP COLUMN IF EXISTS carton_type_id;

-- 5. Drop the legacy catalogue table (verified 0 rows) and its trigger function.
DROP TABLE IF EXISTS public.wms_carton_types CASCADE;
DROP FUNCTION IF EXISTS public._touch_wms_carton_types_updated_at() CASCADE;