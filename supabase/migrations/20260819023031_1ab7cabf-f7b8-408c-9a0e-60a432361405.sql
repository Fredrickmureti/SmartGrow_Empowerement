...
ALTER TABLE public.bank_statements ADD CONSTRAINT bank_statements_status_check
  CHECK (status = ANY (ARRAY['imported'::text, 'reconciling'::text, 'completed'::text, 'failed'::text]));
...
BEGIN
  ...
  -- Statement lifecycle: imported -> reconciling -> completed, or terminal failed.
  -- The whole ingest is one transaction, so no intermediate "processing" state is
  -- ever observable; the header opens as 'imported' and closes below.
  ...
  FOR v_row IN SELECT jsonb_array_elements(_rows) LOOP
    ...
    -- A zero-amount line (typically an "Opening balance" / carry-forward row) is
    -- statement metadata, not bank activity. Storing it creates a line that can
    -- never be reconciled.
    IF v_amount = 0 THEN
      v_rejected := v_rejected + 1;
      v_rejects := v_rejects || jsonb_build_array(jsonb_build_object(
        'row', v_idx, 'reason', 'Zero-amount line is statement metadata, not a bank transaction'));
      CONTINUE;
    END IF;
