...
BEGIN
  ...
  -- Fiscal metadata is per-jurisdiction and lives ONLY in
  -- product_tax_localization. The old etims_* master keys are refused loudly
  -- so a caller can never believe it stored a code that went nowhere.
  IF p_product ?| ARRAY[
       'etims_classification_code','etims_unit_code','etims_packaging_unit',
       'etims_country_origin','etims_origin_country','etims_item_code'
     ] THEN
    RAISE EXCEPTION 'PRODUCT_PAYLOAD_INVALID: etims_* fields are not product master data — send them in p_localization'
      USING ERRCODE = 'P0001';
  END IF;

  v_loc := nullif(coalesce(p_localization, '{}'::jsonb), '{}'::jsonb);
  ...
  -- 6. Lifecycle event: an edit is as observable as a creation.
  IF NOT v_is_new THEN
    PERFORM public.publish_business_event(
      p_org_id         => v_org,
      p_branch_id      => NULL,
      p_warehouse_id   => NULL,
      p_event_type     => 'product.updated',
      p_source_doc_type=> 'product',
      p_source_doc_id  => v_product_id,
      p_payload        => jsonb_build_object(
                            'product_id', v_product_id,
                            'business_id', v_biz
                          ),
      p_idempotency_key=> 'product.updated:' || v_product_id::text || ':' || clock_timestamp()::text
    );
  END IF;
...
-- Lifecycle transitions emit their own event.
CREATE OR REPLACE FUNCTION public.set_product_lifecycle_status(p_product_id uuid, p_status text, p_reason text DEFAULT NULL::text)
RETURNS products
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  SELECT organization_id, status::text INTO v_org, v_prev
  ...
  IF v_prev IS DISTINCT FROM p_status THEN
    PERFORM public.publish_business_event(
      ...
      p_branch_id      => NULL,
      p_warehouse_id   => NULL,
      p_event_type     => 'product.status_changed',
      p_source_doc_type=> 'product',
      p_source_doc_id  => p_product_id,
      p_payload        => jsonb_build_object(
                            'product_id', p_product_id,
                            'business_id', v_row.business_id,
                            'from_status', v_prev,
                            'to_status', p_status,
                            'reason', v_row.lifecycle_reason
                          ),
      p_idempotency_key=> 'product.status:' || p_product_id::text || ':' || p_status || ':' || clock_timestamp()::text
    );
  END IF;
