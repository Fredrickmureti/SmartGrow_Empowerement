DROP VIEW IF EXISTS public.scrap_document_facts;

CREATE VIEW public.scrap_document_facts
...
SELECT
  ...
  first_line.product_id AS first_product_id,
  first_line.product_name AS first_product_name,
  first_line.product_sku AS first_product_sku,
...
COMMENT ON VIEW public.scrap_document_facts IS
  'Canonical read model for scrap/waste dashboards and lists. Security invoker view over scrap stock_adjustments with totals, first product, JE linkage, movement count, and warehouse label.';