...
BEGIN
  ...
  -- Budget authority: the canonical budgets domain, scoped to this project's
  -- analytic account. The projects.budget scalar is informational only and is
  -- used solely when the budgets domain holds nothing for the project.
  IF v_proj.analytic_account_id IS NOT NULL THEN
    SELECT SUM(bi.budgeted_amount), MIN(b.currency_code)
    INTO v_budget, v_budget_currency
    FROM public.budget_items bi
    JOIN public.budgets b ON b.id = bi.budget_id
    WHERE bi.analytic_account_id = v_proj.analytic_account_id
      AND b.status = 'active'
      AND b.business_id = COALESCE(_business_id, v_proj.business_id);
  END IF;

  IF v_budget IS NOT NULL THEN
    v_budget_source := 'budgets_domain';
    v_budget_currency := COALESCE(v_budget_currency, v_base_currency);
  ELSIF v_proj.budget IS NOT NULL THEN
    v_budget := v_proj.budget;
    v_budget_source := 'project_scalar';
    v_budget_currency := COALESCE(v_proj.currency, v_base_currency);
  ELSE
    v_budget_source := 'none';
    v_budget_currency := v_base_currency;
  END IF;
  ...
  RETURN jsonb_build_object(
    ...
    'budget', v_budget,
    'budget_source', v_budget_source,
    'budget_currency', v_budget_currency,
    'budget_used_pct', CASE
      WHEN v_budget IS NULL OR v_budget = 0 THEN NULL
      ELSE round((v_cost / v_budget) * 100, 2)
    END,
