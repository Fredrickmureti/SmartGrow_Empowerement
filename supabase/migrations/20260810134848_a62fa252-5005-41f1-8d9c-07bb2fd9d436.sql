...
BEGIN
  ...
  -- Credit notes: real credit_note_status lifecycle is
  -- draft | issued | applied | void | refunded. There is no 'voided'/'cancelled'
  -- label; referencing them raised 22P02 and returned HTTP 400 for every call.
  ...
  FROM (
    ...
    LEFT JOIN LATERAL (
      ...
      WHERE a.invoice_id = i.id
        AND n.status NOT IN ('draft','void')
    ) cn ON true
