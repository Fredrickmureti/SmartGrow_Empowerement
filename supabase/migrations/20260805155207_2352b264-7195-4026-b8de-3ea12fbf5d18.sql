CREATE OR REPLACE FUNCTION public.upsert_product_identifier(p_business_id uuid, p_product_id uuid, p_code text DEFAULT NULL::text, p_kind product_identifier_kind DEFAULT 'gtin'::product_identifier_kind, p_packaging_id uuid DEFAULT NULL::uuid, p_is_primary boolean DEFAULT false, p_supplier_id uuid DEFAULT NULL::uuid, p_source product_identifier_source DEFAULT NULL::product_identifier_source, p_identifier_id uuid DEFAULT NULL::uuid, p_valid_from timestamp with time zone DEFAULT NULL::timestamp with time zone, p_valid_to timestamp with time zone DEFAULT NULL::timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  IF p_identifier_id IS NOT NULL THEN
    ...
    -- one, never blank it. Same for valid_from, which an edit that only
    -- touches the code must not erase.
