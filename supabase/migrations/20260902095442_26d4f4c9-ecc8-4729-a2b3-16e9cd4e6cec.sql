...
BEGIN
  ...
  INSERT INTO mf_loan_applications (business_id, branch_id, application_number, client_id, product_id,
    ...
    status, submitted_at, submitted_by, approved_amount, approved_term_installments, decision_by, decision_at, created_by)
  VALUES (v_biz, v_branch, 'V1-APP-001', v_client, v_prod, v_ver, v_user, 60000, 4, 'V1 proof',
    'approved', now(), v_user, 48000, 4, v_user, now(), v_user)
  ...
  RETURN jsonb_build_object(
    'ok', true,
    'log', v_log,
    'predecessor', (SELECT to_jsonb(l) - 'created_at' - 'updated_at' FROM mf_loans l WHERE l.id = v_loan),
    'successor_status', (SELECT status FROM mf_loans WHERE id = v_succ),
    'unbalanced_entries', (
      SELECT COALESCE(jsonb_agg(x), '[]'::jsonb) FROM (
        SELECT je.entry_number, je.total_debit, je.total_credit,
               SUM(jel.debit) AS line_debit, SUM(jel.credit) AS line_credit
        FROM journal_entries je JOIN journal_entry_lines jel ON jel.journal_entry_id = je.id
        WHERE je.source_type = 'microfinance' OR je.source_id IN (SELECT id FROM mf_loan_events WHERE loan_id IN (v_loan, v_succ))
        GROUP BY je.id, je.entry_number, je.total_debit, je.total_credit
        HAVING SUM(jel.debit) <> SUM(jel.credit)
      ) x),
    'events', (SELECT COALESCE(jsonb_agg(jsonb_build_object('kind', e.event_kind, 'posted', p.journal_entry_id IS NOT NULL) ORDER BY e.created_at), '[]'::jsonb)
               FROM mf_loan_events e LEFT JOIN mf_event_postings p ON p.event_id = e.id
...
EXCEPTION WHEN OTHERS THEN
  RETURN jsonb_build_object('ok', false, 'failed_step', v_step, 'error', SQLERRM, 'sqlstate', SQLSTATE, 'log', v_log);
END;
$function$;