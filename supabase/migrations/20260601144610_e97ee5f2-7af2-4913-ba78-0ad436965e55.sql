DROP FUNCTION IF EXISTS public.record_multi_invoice_payment(uuid,uuid,uuid,jsonb,numeric,date,text,text,text,text,uuid,uuid,uuid,uuid);

CREATE OR REPLACE FUNCTION public.record_multi_invoice_payment(
  _org_id uuid, _business_id uuid, _contact_id uuid, _allocations jsonb,
  ...
  _receipt_number text DEFAULT NULL, _created_by uuid DEFAULT NULL,
  _deposit_account_id uuid DEFAULT NULL, _receivable_account_id uuid DEFAULT NULL,
  _customer_credit_account_id uuid DEFAULT NULL, _branch_id uuid DEFAULT NULL)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Insert with outstanding_amount = _total_amount so the
  -- enforce_payment_amount_split invariant (amount = outstanding + applied)
  -- holds at INSERT time. We rebalance to the real split after the loop.
  ...
  -- Rebalance the payment row to reflect the actual split (ADR 0012 invariant).
  UPDATE payments
