-- 1. Post negative allocation amounts (credit consumption) as debits.
...
-- 2. Apply any held client credit to the next due installments on the next payment.
...
BEGIN
  ...
  SELECT ROUND(COALESCE(SUM(amount), 0), 2) INTO v_credit
  FROM public.mf_repayment_allocations
  WHERE loan_id = p_loan_id AND component = 'advance' AND repayment_id <> v_repayment_id;
  ...
  -- How much of the held credit was actually consumed by this receipt.
  ...
  -- Anything still unallocated is held as client credit for future installments.
