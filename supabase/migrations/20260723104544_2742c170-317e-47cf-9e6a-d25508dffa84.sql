-- Bring legal-order (garnishment) lifecycle onto the canonical
-- business_event_outbox schema, replacing the legacy direct-INSERT that
-- used non-existent columns (organization_id, business_id, aggregate_*).

-- 1. Register the legal-order topic prefix so consumers can subscribe.
INSERT INTO public.business_event_topics
  (topic_prefix, producer_domain, consumer_domains, handler_scope, description)
VALUES
  ('legal_order.', 'payroll', ARRAY['payroll','finance','audit','analytics'], 'server',
   'Legal order (garnishment) lifecycle: submit, approve, reject, activate, suspend, resume, mark_satisfied, release, expire, terminate_unsatisfied.')
ON CONFLICT (topic_prefix) DO NOTHING;

-- 2. Rewrite garnishment_transition to use the canonical publisher.
...
BEGIN
  ...
  -- Canonical business-event publication. No bespoke INSERT, no
  -- exception swallow: if the outbox write fails, the whole transition
  -- rolls back and no state change is persisted (correct behavior).
