CREATE OR REPLACE FUNCTION public.confirm_invoice_atomic(p_invoice_id uuid, p_user_id uuid, p_main_lines jsonb, p_cogs_lines jsonb DEFAULT NULL::jsonb)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  -- FIX: account_type enum is {asset,liability,equity,income,expense}.
  -- Previously this referenced 'revenue', which caused the RPC to abort with
  -- `invalid input value for enum account_type: "revenue"` on every confirm.
  SELECT
