...
BEGIN
  ...
  -- Billing follows delivery: only a reached milestone is billable.
  IF NOT COALESCE(v_ms.is_reached, false) THEN
    RETURN jsonb_build_object('ok', false, 'error', 'milestone_not_reached');
  END IF;
  ...
  INSERT INTO public.project_activity_log (
    ...
    'milestone.invoiced',
    'Milestone invoiced: ' || v_ms.name || ' (' || v_invoice_number || ')',
    jsonb_build_object('invoice_id', v_invoice_id, 'amount', v_amount, 'currency', v_project.currency)
  );
  ...
  INSERT INTO public.business_event_outbox (
    ...
    'projects.milestone.invoiced', 'project_milestone', v_ms.id,
    jsonb_build_object(
    ...
    'projects.milestone.invoiced:' || v_ms.id::text,
    v_uid,
    'app'
  )
