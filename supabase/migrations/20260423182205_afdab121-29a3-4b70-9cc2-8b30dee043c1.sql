-- Drop every historical overload to guarantee a clean slate
DROP FUNCTION IF EXISTS public.get_sales_dashboard_kpis(uuid, uuid, date, date) CASCADE;
DROP FUNCTION IF EXISTS public.get_sales_dashboard_kpis(uuid, uuid, date, date, uuid) CASCADE;
DROP FUNCTION IF EXISTS public.get_sales_dashboard_kpis(uuid, uuid, uuid, date, date) CASCADE;

-- Canonical definition matching the SalesDashboard.tsx call:
--   p_org_id, p_business_id, p_date_from, p_date_to, p_branch_id (optional)
-- Branch semantics:
--   p_branch_id IS NULL  => company-wide (all branches in the active business)
--   p_branch_id = X      => strictly only branch X (no implicit NULL broadening)
-- Date semantics:
--   p_date_from / p_date_to NULL => period metrics fall back to all-time
...
BEGIN
  ...
  -- AuthZ: use the project's real membership helper (signature: user_id, organization_id)
  ...
  -- Period paid invoice count (by issue_date)
  SELECT COUNT(*)
  ...
  -- Receivable totals (open balance, all-time)
  SELECT
  ...
  -- Aging buckets on the same open invoices
  WITH open_inv AS (
  ...
  -- Top 5 customers (period, by paid revenue from posted invoices)
  ...
  -- Recent payments (last 5 in period)
...
GRANT EXECUTE ON FUNCTION public.get_sales_dashboard_kpis(uuid, uuid, date, date, uuid) TO authenticated;

COMMENT ON FUNCTION public.get_sales_dashboard_kpis(uuid, uuid, date, date, uuid)
  IS 'Sales dashboard KPI aggregator. Uses is_org_member() for AuthZ (organization_members table does not exist).';

-- Force PostgREST to refresh both config and schema cache so the function is callable immediately
