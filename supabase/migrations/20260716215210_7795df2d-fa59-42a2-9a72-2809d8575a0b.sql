...
-- Phase A.2 — Enforcement + RPC wiring for downstream lot/serial stamping
-- ADR 0066.

-- 1. approve_sales_return_atomic — forward lot/serial to CN lines + stock movement
...
-- 2. confirm_invoice_atomic — forward lot/serial into auto-DN lines
CREATE OR REPLACE FUNCTION public.confirm_invoice_atomic(p_invoice_id uuid, p_user_id uuid, p_main_lines jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
-- 3. Enforcement trigger: when a document leaves draft/pending, every
--    lot-tracked-product line must carry a lot_number. Exempt cancelled/rejected.
...
DECLARE
  ...
  v_active boolean;
BEGIN
  -- Determine which line table to check based on TG_TABLE_NAME
  ...
  -- Only check when the doc is entering an active/posted state
  IF NOT v_active THEN RETURN NEW; END IF;
  -- No-op when status is unchanged (bulk updates)
...
DROP TRIGGER IF EXISTS trg_enforce_lot_stamping ON public.invoices;
CREATE TRIGGER trg_enforce_lot_stamping
  BEFORE UPDATE ON public.invoices
  FOR EACH ROW EXECUTE FUNCTION public.enforce_downstream_lot_stamping();

DROP TRIGGER IF EXISTS trg_enforce_lot_stamping ON public.credit_notes;
CREATE TRIGGER trg_enforce_lot_stamping
  BEFORE UPDATE ON public.credit_notes
  FOR EACH ROW EXECUTE FUNCTION public.enforce_downstream_lot_stamping();

DROP TRIGGER IF EXISTS trg_enforce_lot_stamping ON public.sales_returns;
CREATE TRIGGER trg_enforce_lot_stamping
  BEFORE UPDATE ON public.sales_returns
  FOR EACH ROW EXECUTE FUNCTION public.enforce_downstream_lot_stamping();

COMMENT ON FUNCTION public.enforce_downstream_lot_stamping() IS
  'ADR 0066. Blocks a sales-side document from transitioning to a posted status while any lot-tracked-product line is missing lot_number.';
