...
BEGIN
  ...
  WITH pending AS (
    ...
    RETURNING (render_params->>'line_id')::uuid AS line_id
  )
  UPDATE public.label_print_run_lines l
...
SET status = 'expanding', last_error = NULL, updated_at = now()
WHERE status = 'failed'
  AND last_error LIKE '%organization_id%';