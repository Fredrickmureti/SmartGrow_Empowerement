...
BEGIN
  ...
  -- 1) default_account_settings — match the same priority useDefaultAccounts uses:
  --    business-scoped wins, then org-scoped (null business_id), then ANY row for the org
  ...
  IF _business_id IS NOT NULL THEN
    ...
    ORDER BY (business_id = _business_id) DESC
    LIMIT 1;
  ELSE
    SELECT account_id
      INTO v_control_account_id
    ...
    WHERE organization_id = _org_id
      AND setting_key = v_setting_key
    ORDER BY (business_id IS NULL) DESC
    LIMIT 1;
  END IF;

  -- 2) Legacy default_account_mappings fallback.
  IF v_control_account_id IS NULL THEN
    IF _business_id IS NOT NULL THEN
      ...
      WHERE organization_id = _org_id
        AND mapping_type = v_setting_key
        AND (business_id = _business_id OR business_id IS NULL)
      ORDER BY (business_id = _business_id) DESC
      LIMIT 1;
    ELSE
