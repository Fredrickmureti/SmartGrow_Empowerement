-- =====================================================================
-- Smart approval routing for stock adjustments
-- =====================================================================

-- 1) Index for fast approval-rule lookup
CREATE INDEX IF NOT EXISTS idx_approval_rules_entity_action
  ON public.approval_rules (organization_id, entity_type, action_name)
  WHERE is_active = true;

-- 2) Update approve_stock_adjustment_atomic:
--    - accept either 'draft' or 'pending_approval' as the entry state
--    - refuse to act on already 'approved' rows (idempotent)
--    - mark related approval_rule_logs entry as approved (if any)
...
-- 3) New routing function: decides auto-apply vs pending_approval
--    p_input shape:
--    {
--      "organization_id": "...", "business_id": "...", "branch_id": "...",
--      "warehouse_id": "...", "adjustment_number": "...",
--      "reason": "...", "notes": "...",
--      "items": [
--        { "product_id":"...", "warehouse_id":"...",
--          "quantity_adjustment": 5, "unit_cost": 10.5, "notes": "..." }, ...
--      ]
--    }
...
BEGIN
  ...
  -- Permission check (same authority as RLS)
  ...
  -- Resolve the user's effective role to decide auto-approver eligibility
  ...
  -- Compute totals over the input lines (without touching the DB yet)
  ...
  -- Evaluate active approval rules for stock_adjustment.apply
  FOR v_rule IN
    SELECT *
     ...
     WHERE organization_id = v_org_id
       ...
       AND is_active = true
  LOOP
    v_threshold_match := true; -- no threshold defined => always triggers
    ...
    IF v_threshold_match THEN
      -- Privileged users (org owner/admin) bypass the rule
  ...
  -- Insert the header in the resolved status
  ...
  -- Insert items with quantity_before snapshot
  FOR v_item IN SELECT jsonb_array_elements(v_items) LOOP
  ...
  IF v_auto_approve THEN
    -- One-click apply: post movements + GL right now
  ...
  ELSE
    ...
    INSERT INTO public.approval_rule_logs (
      ...
      'stock_adjustment', v_adjustment_id, 'apply',
      'pending', p_user_id,
...
GRANT EXECUTE ON FUNCTION public.apply_or_request_stock_adjustment(jsonb, uuid)
  TO authenticated;

COMMENT ON FUNCTION public.apply_or_request_stock_adjustment(jsonb, uuid) IS
  'Single entry-point for creating a stock adjustment. Evaluates approval_rules for entity_type=stock_adjustment, action_name=apply. Auto-applies (posts movements + GL) when no rule triggers or the caller is org owner/admin. Otherwise stores the adjustment as pending_approval and creates a matching approval_rule_logs row.';