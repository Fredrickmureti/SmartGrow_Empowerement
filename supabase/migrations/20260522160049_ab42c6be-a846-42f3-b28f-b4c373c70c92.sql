-- =====================================================================
-- Wave 11 — Opening stock GL integrity + product_categories dedupe
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1. approve_stock_adjustment_atomic — drop the zero-cost carve-out
--    Opening-balance lines must now carry a positive unit_cost.
--    Everything else (lock order, per-line cost persistence, reason-keyed
--    offsets, multi-warehouse handling) is preserved verbatim from the
--    Wave 10 version (20260522154328).
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  LOOP
    ...
    ELSE
      ...
      -- Strict valuation: every non-zero adjustment line — including
      -- opening_balance — must carry a positive unit cost. Quantity-only
      -- inventory events are not permitted because they would update
      -- warehouse_stock without producing a balanced journal entry.
...
COMMENT ON FUNCTION public.approve_stock_adjustment_atomic(uuid, uuid) IS
  'Wave 11 — removes the opening_balance zero-cost carve-out. Every non-zero adjustment line, including opening stock, must carry a positive unit_cost so the GL stays balanced. Opening-balance lines are still routed to Opening Balance Equity by resolve_adjustment_offset_account.';

-- ---------------------------------------------------------------------
-- 2. create_product_with_opening_stock_atomic — require unit_cost > 0
--    on every opening line BEFORE inserting the product.
-- ---------------------------------------------------------------------
...
BEGIN
  ...
  -- Validate every positive opening line: warehouse must exist + belong to
  -- the same org/business, NOT be in-transit, have a branch, AND carry a
  -- positive unit cost (caller-supplied or product cost_price). Failing
  -- fast here gives the UI a clean error instead of relying on rollback.
...
COMMENT ON FUNCTION public.create_product_with_opening_stock_atomic(jsonb, jsonb, uuid) IS
  'Wave 11 — pre-validates every positive opening line for a positive unit cost so opening stock cannot be created as a quantity-only event. Pairs with approve_stock_adjustment_atomic which posts the resulting JE against Opening Balance Equity.';

-- ---------------------------------------------------------------------
-- 3. Backfill — merge duplicate product_categories rows in place
--    Strategy: per (organization_id, business_id, lower(name), parent_id)
--    group with COUNT(*) > 1, keep the oldest row as canonical and
--    re-point products + child categories to it. Snapshot the deleted
--    rows into audit_logs so the merge is auditable / reversible.
-- ---------------------------------------------------------------------
DO $merge$
DECLARE
  v_group RECORD;
...
BEGIN
  -- Make sure audit_logs exists; if not, skip the snapshot but still merge.
  PERFORM 1 FROM information_schema.tables
   WHERE table_schema='public' AND table_name='audit_logs';

  FOR v_group IN
    SELECT organization_id, business_id, lower(name) AS lname,
           COALESCE(parent_id, '00000000-0000-0000-0000-000000000000'::uuid) AS pid_key
  ...
  LOOP
    SELECT id INTO v_canonical_id
     ...
     WHERE organization_id = v_group.organization_id
       AND business_id = v_group.business_id
       AND lower(name) = v_group.lname
       AND COALESCE(parent_id, '00000000-0000-0000-0000-000000000000'::uuid) = v_group.pid_key
       AND is_active = true
    ...
    LOOP
      ...
      UPDATE public.products
       ...
       WHERE category_id = v_dup.id;

      UPDATE public.product_categories
         SET parent_id = v_canonical_id
       WHERE parent_id = v_dup.id;

      -- Audit log (best-effort; some tenants may not have the table)
      BEGIN
        INSERT INTO public.audit_logs (
          organization_id, business_id, actor_id, action,
          entity_type, entity_id, metadata, created_at
        )
        VALUES (
          v_dup.organization_id, v_dup.business_id, NULL,
          'product_category_dedupe_merge',
          'product_category', v_dup.id,
          jsonb_build_object(
            'canonical_id', v_canonical_id,
            'merged_row', v_snapshot,
            'reason', 'Wave 11 duplicate cleanup'
          ),
          now()
        );
      EXCEPTION WHEN OTHERS THEN
        RAISE NOTICE 'audit_logs insert skipped for dup %: % (%)', v_dup.id, SQLERRM, SQLSTATE;
      END;

      DELETE FROM public.product_categories WHERE id = v_dup.id;
      v_total_merged := v_total_merged + 1;
    END LOOP;
  END LOOP;

  RAISE NOTICE 'Wave 11 — merged % duplicate product_categories rows', v_total_merged;
END
$merge$;

-- ---------------------------------------------------------------------
-- 4. Hard uniqueness constraint — prevents future duplicates from any
--    insertion path (UI hook, CSV resolver, sample-data seed, RPC).
--    Case-insensitive; scoped to active rows; treats NULL parent as a
--    sentinel zero-UUID so top-level dupes are also blocked.
-- ---------------------------------------------------------------------
CREATE UNIQUE INDEX IF NOT EXISTS product_categories_unique_active_name
...
COMMENT ON INDEX public.product_categories_unique_active_name IS
  'Wave 11 — structural guarantee that no two active product_categories rows share (organization_id, business_id, lower(name), parent_id). Makes render-time deduplication unnecessary; CategoryResolver and the create mutation already lookup-then-insert, so this index is the backstop.';
