...
COMMENT ON COLUMN public.pos_transactions.snapshot IS
  'Immutable JSON snapshot of the transaction at commit time: items (with name, unit_price, tax, line_total snapshotted from products at the moment of sale), payments, customer, cashier, register, branch, currency, totals, committed_at. Reprints and audits MUST read this column, not joins to live product/template tables.';
...
CREATE OR REPLACE FUNCTION public.process_pos_transaction(
  ...
  p_customer_id uuid DEFAULT NULL, p_customer_tin text DEFAULT NULL, p_customer_name text DEFAULT NULL,
...
BEGIN
  ...
  -- STAGE 3 GUARD: a paid sale with a positive total must carry at least one
  -- payment. Without this the RPC would silently mark it 'partial' and create
  -- a phantom AR-style obligation with no tender to reconcile against.
  ...
  -- Resolve currency for snapshot (best-effort; falls back to NULL)
  SELECT currency_code INTO v_currency FROM public.businesses WHERE id = v_register_business_id;

  -- Build the immutable snapshot. Items are taken straight from the input
  -- payload (already carries name/unit_price/tax_amount/line_total at the
  -- moment the cashier rang the sale).
