DROP FUNCTION IF EXISTS public.create_journal_entry_atomic(uuid,uuid,text,date,text,text,boolean,boolean,uuid,jsonb);

CREATE FUNCTION public.create_journal_entry_atomic(
  _org_id uuid,
  _business_id uuid,
  _entry_number text,
  _entry_date date,
