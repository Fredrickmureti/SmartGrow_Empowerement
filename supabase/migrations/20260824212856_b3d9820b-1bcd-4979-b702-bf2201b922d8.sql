-- CRM Phase 1 (completion): convert_lead_to_sales_order was missed by the
-- first Phase 1 migration and still carried D1 (crm_activities insert without
-- business_id → every conversion rolled back) and D2 (no caller authorization).
...
REVOKE ALL ON FUNCTION public.convert_lead_to_sales_order(uuid, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.convert_lead_to_sales_order(uuid, uuid) TO authenticated, service_role;