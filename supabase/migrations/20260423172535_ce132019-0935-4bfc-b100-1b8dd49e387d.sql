-- Drop all existing overloads of get_sales_dashboard_kpis
DROP FUNCTION IF EXISTS public.get_sales_dashboard_kpis(uuid, uuid, date, date);
DROP FUNCTION IF EXISTS public.get_sales_dashboard_kpis(uuid, uuid, date, date, uuid);
DROP FUNCTION IF EXISTS public.get_sales_dashboard_kpis(uuid, uuid, uuid, date, date);

-- One canonical function returning the exact JSON contract SalesDashboard.tsx expects.
-- Branch semantics:
...
-- Date semantics:
--   p_date_from / p_date_to NULL => period-scoped metrics fall back to all-time
...
DECLARE
  ...
  -- Pipeline counts (all-time, scoped)
  v_draft_n   bigint := 0;
  v_sent_n    bigint := 0;
  v_overdue_n bigint := 0;
  v_paid_n    bigint := 0;

  -- Period-scoped paid invoice count
  v_period_paid_n bigint := 0;

  -- Receivable (open balance, all-time)
...
BEGIN
  ...
  -- Pipeline (all-time, branch-strict)
  SELECT
  ...
  -- Sales orders pending fulfillment (not delivered/cancelled/converted)
  SELECT COUNT(*)
  INTO v_so_pending
  ...
  -- Credit notes (period by issue_date)
