...
BEGIN
  ...
  -- Resolve the business-local "today" so dashboards filtering by
  -- attendance_date in business-local time match the inserted row.
  v_tz := COALESCE(
    (SELECT NULLIF(timezone, '') FROM public.businesses WHERE id = v_business),
    'UTC'
  );
  BEGIN
  ...
  IF v_deny IS NULL AND _lat IS NOT NULL AND _lng IS NOT NULL
    ...
    IF v_last.lat IS NOT NULL THEN
      ...
      IF v_speed_kmh > v_settings.max_speed_kmh THEN
        ...
        ELSE
          v_review_reasons := array_append(v_review_reasons, 'IMPOSSIBLE_TRAVEL');
  ...
  IF v_deny IS NULL AND COALESCE(v_settings.enforce_shift_window, false) THEN
    ...
    IF v_shift.start_time IS NOT NULL THEN
      IF (v_now AT TIME ZONE v_tz)::time < v_shift.start_time - make_interval(mins => COALESCE(v_settings.early_clock_in_minutes,30))
         OR (v_now AT TIME ZONE v_tz)::time > v_shift.start_time + make_interval(mins => COALESCE(v_settings.late_clock_in_minutes,120)) THEN
  ...
  IF v_deny IS NULL AND COALESCE(v_settings.device_binding_required, false) THEN
    IF _device_fp IS NULL THEN
      ...
      ELSE
        v_review_reasons := array_append(v_review_reasons, 'NO_DEVICE_FINGERPRINT');
      END IF;
    ELSE
      ...
      IF v_trust.id IS NULL THEN
        ...
        ELSE
          ...
          v_review_reasons := array_append(v_review_reasons, 'NEW_DEVICE_AUTOTRUSTED');
        END IF;
  ...
  IF COALESCE(array_length(v_review_reasons,1),0) > 0 THEN
    DECLARE r text;
    BEGIN
      FOREACH r IN ARRAY v_review_reasons LOOP
        IF r <> 'IMPOSSIBLE_TRAVEL' THEN
          PERFORM public.attendance_log_event(
