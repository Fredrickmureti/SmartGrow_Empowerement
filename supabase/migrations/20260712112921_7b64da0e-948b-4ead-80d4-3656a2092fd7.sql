...
BEGIN
  ...
  ELSE
    ...
    IF jsonb_typeof(_blocks) = 'array' AND jsonb_array_length(_blocks) > 0 THEN
      SELECT EXISTS (
        SELECT 1 FROM jsonb_array_elements(_blocks) elem
        WHERE elem->>'type' = 'field_grid' AND elem->>'data_source' = 'employer'
      ) INTO _has_employer;
      SELECT EXISTS (
        SELECT 1 FROM jsonb_array_elements(_blocks) elem
        WHERE elem->>'type' = 'field_grid' AND elem->>'data_source' = 'employee'
      ) INTO _has_employee;
      SELECT EXISTS (
        SELECT 1 FROM jsonb_array_elements(_blocks) elem
        WHERE elem->>'type' = 'signature_block'
      ) INTO _has_signature;
      SELECT EXISTS (
        SELECT 1 FROM jsonb_array_elements(_blocks) elem
        WHERE elem->>'type' = 'table'
          AND COALESCE(elem->>'data_source','') IN ('monthly_breakdown','monthly_matrix','ytd_rows')
      ) INTO _has_data;
...
UPDATE public.pack_rule_type_schemas
   SET json_schema = jsonb_set(
     json_schema,
     '{required}',
     '["data_source","blocks"]'::jsonb,
     true
   )
 WHERE rule_type = 'certificate_template'
   AND computation_kind = 'v2'
   AND jsonb_typeof(json_schema->'properties'->'blocks') = 'object';