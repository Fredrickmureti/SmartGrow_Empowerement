...
BEGIN
  -- 1) Outstanding liabilities by authority.
  --    Note: payroll_liabilities does not carry a per-row currency in this
  --    schema; currency is resolved from the organization at render time.
  ...
  --    payroll_remittance_payments does not carry a currency_code column;
  --    render layer resolves currency from the organization.
  SELECT COALESCE(jsonb_agg(row_to_json(t) ORDER BY (t).payment_date ASC), '[]'::jsonb)
    INTO v_uncleared_payments
  FROM (
    SELECT
      p.id, p.payment_date, p.total_amount,
      NULL::text AS currency_code,
