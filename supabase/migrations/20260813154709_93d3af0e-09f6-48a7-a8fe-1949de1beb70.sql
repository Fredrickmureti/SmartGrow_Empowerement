DELETE FROM public.business_event_topics WHERE topic_prefix = 'landed_cost.';

CREATE OR REPLACE FUNCTION public._emit_landed_cost_outbox(_voucher_id uuid, _state text, _payload jsonb)
RETURNS void
LANGUAGE plpgsql
...
BEGIN
  SELECT organization_id, branch_id INTO v_org, v_branch
    FROM public.landed_cost_vouchers WHERE id = _voucher_id;
  INSERT INTO public.business_event_outbox
    (org_id, branch_id, source, event_type, source_doc_type, source_doc_id,
     payload, idempotency_key, status, actor_user_id, created_at)
  VALUES (v_org, v_branch, 'procurement',
          'procurement.landed_cost.' || _state,
          'landed_cost_voucher', _voucher_id, COALESCE(_payload, '{}'::jsonb),
          'procurement.landed_cost.' || _state || ':' || _voucher_id::text,
          'pending', auth.uid(), now())
  ON CONFLICT (idempotency_key) DO NOTHING;
END $fn$;

CREATE OR REPLACE FUNCTION public.landed_cost_post_voucher(p_voucher_id uuid, p_actor uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
CREATE OR REPLACE FUNCTION public.landed_cost_reverse_voucher(p_voucher_id uuid, p_reason text, p_actor uuid DEFAULT NULL::uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
