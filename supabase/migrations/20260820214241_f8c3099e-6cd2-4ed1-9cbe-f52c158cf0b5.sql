...
BEGIN
  ...
  WITH txn_all AS (
    -- Every statement line of the account, unbounded: an entry linked to any
    -- of them is matched, never an outstanding item.
    SELECT bt.id, bt.journal_entry_id
      FROM public.bank_transactions bt
     WHERE bt.bank_account_id = a.id
  ),
  txn AS (
    -- The lines that belong to the balance being proved.
    SELECT bt.id,
           bt.transaction_date,
           bt.description,
           bt.reference,
