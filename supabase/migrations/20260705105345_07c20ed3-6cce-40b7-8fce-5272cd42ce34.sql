-- Fix schema drift in tg_hr_event_propose_loan_skip: employee_lifecycle_events
-- has no `created_by` column (it uses `actor_user_id`). The stale reference
-- broke every INSERT into employee_lifecycle_events, cascading into every HR
-- write that emits a lifecycle event (contract create/activate/expire, hire,
-- termination, leave start/end, reinstate).
