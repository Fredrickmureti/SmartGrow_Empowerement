-- Fix journal entry number generation in record_payment_atomic
-- Use proper sequence-based entry number generation to prevent duplicates
...
BEGIN
  ...
  IF _cash_account_id IS NOT NULL AND _receivable_account_id IS NOT NULL THEN
    ...
    IF v_je_id IS NULL THEN
      -- Generate proper entry number using sequence
      IF _je_entry_number IS NOT NULL THEN
        v_entry_number := _je_entry_number;
      ELSE
        SELECT get_next_journal_entry_number(_org_id) INTO v_entry_number;
      END IF;
      ...
      INSERT INTO journal_entries (
        ...
        _org_id, _business_id, v_entry_number, _payment_date, 
        _receipt_number, 'Payment received - ' || COALESCE(_receipt_number, 'Payment'),
        'payment', v_payment_id, 'posted', _created_by
      ) RETURNING id INTO v_je_id;

      -- Debit Cash/Bank
      INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit, credit, description)
      VALUES (v_je_id, _cash_account_id, _amount, 0, 'Payment ' || COALESCE(_receipt_number, 'Payment') || ' - Cash Receipt');

      -- Credit Accounts Receivable
      INSERT INTO journal_entry_lines (journal_entry_id, account_id, debit, credit, description)
      VALUES (v_je_id, _receivable_account_id, 0, _amount, 'Payment ' || COALESCE(_receipt_number, 'Payment') || ' - AR Reduction');
    END IF;
  END IF;
