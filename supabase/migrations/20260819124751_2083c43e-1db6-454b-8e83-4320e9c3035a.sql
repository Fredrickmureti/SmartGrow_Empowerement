-- =====================================================================
-- Reconciliation engine · Phase 10 — feed → matching automation hardening.
--
-- ADR-0147 §4/§6: an explanation is evidence, and an automation may only
-- post what is unambiguous. Five defects closed here:
--
--   F12  bank_match_candidates offered candidates for a bank line that is
--        already reconciled — a statement re-import (or a feed replay)
--        surfaced a "deterministic" suggestion for settled money.
--   F13  a line already carrying a proposed match got a fresh candidate
--        set, so one line could accumulate competing proposals.
--   F14  documents already spoken for by a PROPOSED match were still
--        offered, so the same invoice/receipt could be proposed on two
--        bank lines at once.
--   F15  the multi-receipt deposit query aggregated ALL valid pairs into
--        one row, so two possible combinations produced NO candidate at
--        all instead of an ambiguous pair of them.
--   F16  apply_reconciliation_rules re-proposed lines that already had a
--        proposal, and auto-posted when two rules of equal priority both
--        claimed the line.
-- =====================================================================
...
BEGIN
  ...
  -- F12/F13. A line that is already explained is not a question. Suggesting
  -- against settled money is how a replayed statement becomes a double
  -- settlement, and suggesting against an open proposal is how one line
  -- grows two competing ones.
  SELECT m.id, m.status INTO _open
    FROM public.bank_reconciliation_matches m
   WHERE m.bank_transaction_id = _txn_id
     AND m.status IN ('confirmed', 'proposed')
  ...
  IF _open.id IS NOT NULL OR COALESCE(_txn.is_reconciled, false) THEN
    RETURN jsonb_build_object(
      ...
      'tier', CASE WHEN _open.status = 'proposed' THEN 'proposed' ELSE 'settled' END,
      'existing_match_id', _open.id,
      'candidates', '[]'::jsonb,
      'reason', CASE WHEN _open.status = 'proposed'
  ...
  IF _inflow THEN
    ...
    -- 1b. Several recorded receipts banked as one deposit. F15: each valid
    --     combination is its own candidate, so two possibilities read as a
    --     tie (ambiguous) instead of cancelling each other out.
    FOR _r IN
      WITH pool AS (
  ...
  -- 3. A transfer from one of our own accounts. The mirror line must itself
  --    be unexplained — a mirror already proposed elsewhere is not free.
  FOR _r IN
...
-- F14. One question, asked once: is this document already spoken for by a
-- proposed or confirmed match? Stated once so every candidate class asks it
-- the same way.
...
REVOKE ALL ON FUNCTION public.bank_match_candidates(uuid, integer) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.bank_match_candidates(uuid, integer) TO authenticated;

-- =====================================================================
-- F16 — the rules runner may not re-propose, and may not auto-post a guess.
-- =====================================================================
