-- ============================================================
-- Carton runs (Phase 6 item 6)
--
-- PackStation prints one carton at a time through the sanctioned
-- per-entity seam, which is correct for a single carton. A 40-carton
-- shipment is a batch, and a batch belongs to the engine.
-- ============================================================
CREATE OR REPLACE FUNCTION public.label_vars_for_carton(
  p_business_id uuid,
  p_carton_id uuid
) RETURNS jsonb
LANGUAGE sql STABLE SECURITY DEFINER SET search_path = public AS $$
  SELECT jsonb_build_object(
    'carton_id', c.id,
    'name', 'Carton ' || COALESCE(lp.code, left(c.id::text, 8)),
    'lpn_code', COALESCE(lp.code, ''),
    -- ADR-0089: the printable identity is the licence plate, never the row id.
    'barcode', COALESCE(NULLIF(lp.code, ''), ''),
    'sku', COALESCE(lp.code, ''),
    'weight_kg', COALESCE(c.weight_kg, 0),
    'length_cm', COALESCE(c.length_cm, 0),
    'width_cm', COALESCE(c.width_cm, 0),
    'height_cm', COALESCE(c.height_cm, 0),
  ...
  WHERE c.id = p_carton_id AND c.business_id = p_business_id;
$$;

GRANT EXECUTE ON FUNCTION public.label_vars_for_carton(uuid, uuid) TO authenticated, service_role;

-- Two new selection kinds: explicit carton ids, and "every sealed
-- carton on this wave" so the operator never enumerates them.
