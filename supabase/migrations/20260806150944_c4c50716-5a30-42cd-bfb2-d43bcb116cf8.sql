...
BEGIN
  ...
  IF v_txn.customer_id IS NULL THEN
    RAISE EXCEPTION 'create_pos_credit_sale_invoice_atomic: credit sale % has no customer', _pos_transaction_id
      USING ERRCODE = 'invalid_parameter_value';
  END IF;
  ...
  -- Credit amount is derived server-side from the recorded tenders,
  ...
  INSERT INTO public.invoices (
    ...
    currency, notes, created_by, salesperson_id
  ) VALUES (
    v_txn.organization_id, v_txn.business_id, v_txn.branch_id, v_txn.customer_id,
    v_invoice_no, 'draft'::invoice_status, 'pos',
    CURRENT_DATE, CURRENT_DATE + COALESCE(_due_days, 30),
    v_subtotal, v_tax, COALESCE(v_txn.discount_amount, 0), v_credit_amount, 0,
    v_currency,
...
REVOKE ALL ON FUNCTION public.create_pos_credit_sale_invoice_atomic(uuid, integer, uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.create_pos_credit_sale_invoice_atomic(uuid, integer, uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.create_pos_credit_sale_invoice_atomic(uuid, integer, uuid) TO service_role;