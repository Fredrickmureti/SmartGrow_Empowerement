...
-- ============================================================
-- Phase 3 · Batch T1 — POS server-authoritative money math
-- ADR 0082 D1
-- ============================================================

-- 1) Canonical line resolver ---------------------------------------------------
...
BEGIN
  IF p_product_id IS NULL THEN
    -- non-product line: trust submitted unit price, no tax resolution
    v_unit_price := COALESCE(p_requested_unit_price, 0);
  ELSE
    SELECT COALESCE(unit_price, 0), COALESCE(tax_rate, 0), tax_rate_id
      INTO v_catalog_price, v_tax_rate, v_tax_rate_id
      FROM public.products
     WHERE id = p_product_id AND business_id = p_business_id
     LIMIT 1;

    -- If a rate row is bound, prefer its rate over the denormalised product.tax_rate
    IF v_tax_rate_id IS NOT NULL THEN
      SELECT COALESCE(rate, v_tax_rate) INTO v_tax_rate
      FROM public.tax_rates WHERE id = v_tax_rate_id LIMIT 1;
    END IF;

    v_unit_price := v_catalog_price;
  END IF;

  -- Discount policy: percentage or fixed applied to the pre-tax line subtotal
  IF p_discount_type = 'percentage' THEN
    v_discount_amount := ROUND((v_unit_price * v_qty) * COALESCE(p_discount_value, 0) / 100.0, 4);
  ELSIF p_discount_type = 'fixed' THEN
    v_discount_amount := LEAST(v_unit_price * v_qty, COALESCE(p_discount_value, 0));
  ELSE
    v_discount_amount := 0;
  END IF;

  v_taxable := GREATEST(0, (v_unit_price * v_qty) - v_discount_amount);
  v_tax_amount := ROUND(v_taxable * v_tax_rate / 100.0, 4);
...
REVOKE ALL ON FUNCTION public.pos_resolve_line(uuid,uuid,numeric,numeric,text,numeric) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.pos_resolve_line(uuid,uuid,numeric,numeric,text,numeric) TO authenticated, service_role;

-- 2) Server-authoritative commit RPC ------------------------------------------
...
BEGIN
  ...
  -- --- Stock availability (unchanged for T1; T4 will replace with FOR UPDATE) ---
  ...
  -- ============================================================
  --  Batch T1 · Server-authoritative money math
  -- ============================================================
  -- Iterate lines: resolve canonical values, detect deviation, enforce
  -- manager override, aggregate the server-side totals.
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_items) LOOP
    v_product_id := NULLIF(v_item->>'product_id', '')::uuid;
    v_quantity := COALESCE((v_item->>'quantity')::numeric, 0);
    ...
    -- Deviation check applies only to product lines; ad-hoc lines
    -- (no product) are trusted because there is no catalog to check against.
    IF v_product_id IS NOT NULL THEN
      ...
      IF v_override_required THEN
        ...
        -- Honour the authorised deviation: use the client-supplied price/discount.
    ...
    -- Consume the override if it was used
  ...
  -- Payments must cover the SERVER-computed total (not client total).
  ...
  -- Persist lines using the RESOLVED (server) values
  v_item_index := 0;
  ...
  -- Shift roll-up uses SERVER totals
  ...
  RETURN jsonb_build_object(
    ...
    -- T1: expose server-authoritative totals for UX reconciliation
    'server_totals', jsonb_build_object(
      'subtotal', v_srv_subtotal, 'tax_amount', v_srv_total_tax,
      'discount_amount', v_srv_total_discount, 'total', v_srv_total),
    'client_totals', jsonb_build_object(
