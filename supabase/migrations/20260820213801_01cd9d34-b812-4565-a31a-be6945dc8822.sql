...
BEGIN
  ...
  IF NOT FOUND THEN
    -- Also the answer for an account owned by another organization: the
    -- caller learns nothing about its existence.
    RAISE EXCEPTION 'bank account not found in this organization'
      USING ERRCODE = '42501';
  END IF;

  -- A ledger account shared by several bank accounts cannot be attributed to
  -- one of them; the book side is reported as unavailable rather than guessed.
  ...
  totals AS (
    SELECT
      agg.*,
      (a.opening_balance + agg.statement_lines_net)                                     AS statement_balance,
      (a.opening_balance + agg.statement_lines_net
  ...
  SELECT jsonb_build_object(
    ...
    'bank', jsonb_build_object(
      'opening_balance',      round(a.opening_balance, 2),
...
COMMENT ON FUNCTION public.finance_bank_reconciliation_statement(uuid, uuid, date, uuid, uuid) IS
  'Bank reconciliation statement for one bank account as at a date: statement balance -> deposits in transit -> unpresented payments -> adjusted bank balance vs GL book balance -> unrecorded statement items -> adjusted book balance -> residual. Single account, single currency; never a cross-currency sum. Caller must be a member of _org_id.';