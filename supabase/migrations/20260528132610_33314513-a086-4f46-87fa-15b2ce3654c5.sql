-- Fix install_localization_pack_atomic: tax_rates schema drift.
-- The function previously omitted tax_type, fixed_amount, effective_from,
-- all of which are NOT NULL on tax_rates with no defaults, causing every
-- install to fail with 23502 on the very first INSERT.
--
-- Also:
--   * mark pack-installed rules as validated (legacy_unvalidated = false)
--     since they come from a published pack
--   * record the source pack template id for upgrade/diff tracking
--   * return a structured error payload on failure so the edge function can
--     surface a precise message to the UI instead of a generic 500
...
BEGIN
  ...
  -- Seed tax rates. Schema requires tax_type / fixed_amount / effective_from;
  -- template provides tax_type (defaulted), and we set the rest to safe values.
  ...
  -- Seed accounts. Map legacy 'revenue' → 'income' to satisfy the enum.
  _step := 'seed_accounts';
  WITH inserted AS (
...
EXCEPTION WHEN OTHERS THEN
  ...
  -- Re-raise with structured context so the edge function can extract
  -- sqlstate / step / detail and map to a UI-friendly code.
  RAISE EXCEPTION
