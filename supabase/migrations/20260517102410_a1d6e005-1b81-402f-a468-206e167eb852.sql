-- 1. Auto-DN insert: always set source_invoice_id + default received_by_contact_id
...
BEGIN
  ...
  -- Stamp source_invoice_id on a SO-spawned DN if one already exists
  IF v_inv.source_sales_order_id IS NOT NULL THEN
    UPDATE public.delivery_notes
       SET source_invoice_id = p_invoice_id
     WHERE sales_order_id = v_inv.source_sales_order_id
       AND business_id = v_inv.business_id
       AND source_invoice_id IS NULL;
  END IF;

  -- Auto-create a DN only when invoice has no SO link
    ...
    IF v_existing_dn_id IS NULL THEN
      ...
      IF v_stockable_count > 0 THEN
        ...
        INSERT INTO public.delivery_notes (
          ...
          NULL, p_invoice_id, v_inv.contact_id,
          NULL, p_user_id
        )
...
-- 2. One-time backfill: stamp source_invoice_id on existing SO-spawned DNs
UPDATE public.delivery_notes dn
   SET source_invoice_id = so.converted_invoice_id
  FROM public.sales_orders so
 WHERE dn.sales_order_id = so.id
   AND so.converted_invoice_id IS NOT NULL
   AND dn.source_invoice_id IS NULL
   AND dn.business_id = so.business_id;

-- 3. Backfill receiver-side contact for existing auto-DNs (only when unset)
UPDATE public.delivery_notes
   SET received_by_contact_id = contact_id
 WHERE received_by_contact_id IS NULL
