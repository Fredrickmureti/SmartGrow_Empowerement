...
-- 1. Add missing column to pos_transaction_payments
ALTER TABLE pos_transaction_payments 
ADD COLUMN IF NOT EXISTS mpesa_receipt_number TEXT;

-- 2. Drop old 15-param overload
DROP FUNCTION IF EXISTS process_pos_transaction(
  UUID, UUID, UUID, JSONB, JSONB, NUMERIC, NUMERIC, NUMERIC, NUMERIC, 
  UUID, TEXT, TEXT, TEXT, UUID, UUID
);

-- 3. Drop and recreate the 20-param function with fixes
DROP FUNCTION IF EXISTS process_pos_transaction(
  UUID, UUID, UUID, JSONB, JSONB, NUMERIC, NUMERIC, NUMERIC, NUMERIC,
  ...
  LOOP
    ...
    IF v_product_id IS NOT NULL AND p_transaction_type = 'sale' THEN
      ...
      IF v_track_inventory = true THEN
        ...
        -- FIX: Use stock_movements (correct table) instead of inventory_movements
