...
BEGIN
  ...
  END IF;
  IF ba.currency IS NULL OR btrim(ba.currency) = '' THEN
    RAISE EXCEPTION 'Bank account % has no currency configured; set its currency before banking a collection', ba.name;
  END IF;
