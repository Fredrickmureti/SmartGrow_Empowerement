...
BEGIN
  ...
  -- Architectural guard: jsonb_populate_record casts every key to its column
  -- type, so a blank string in the payload (common from HTML form inputs)
  -- becomes an invalid cast for date/numeric/uuid columns. Normalise empty
  -- string scalars to JSON null up-front so every caller (form, drafts,
  -- import, future RPCs) benefits.
  FOR v_key IN SELECT k FROM jsonb_object_keys(p_employee) AS k LOOP
    IF jsonb_typeof(p_employee->v_key) = 'string' AND (p_employee->>v_key) = '' THEN
