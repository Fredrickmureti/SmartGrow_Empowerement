-- Phase E (D-13): Harden POS shift GL posting idempotency.
--
-- Even though `post_journal_entry_atomic` enforces uniqueness on
-- (org, source_type, source_id, COALESCE(source_subtype,'main')),
-- the trigger `generate_pos_shift_journal_entry` swallowed any
-- exception path silently. Add an explicit early-return when a JE
-- already exists for this shift, so re-runs of the trigger or the
-- `replay_pos_shift_gl` RPC are guaranteed safe.
...
BEGIN
  ...
  -- Idempotency guard: if this shift is already linked to a JE, return it.
  -- This makes the function safe to re-invoke from triggers, replay RPC,
  -- or any future retry path.
