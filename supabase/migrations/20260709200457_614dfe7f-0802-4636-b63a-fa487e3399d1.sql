...
BEGIN
  ...
  -- The stock_adjustments row is a derived side-effect of a governed Physical Count.
  -- Its approval trigger still runs, so stamp a different count lifecycle actor as
  -- the adjustment creator when possible. The real poster remains recorded on the
  -- physical_count event, stock_movements, and journal entry.
