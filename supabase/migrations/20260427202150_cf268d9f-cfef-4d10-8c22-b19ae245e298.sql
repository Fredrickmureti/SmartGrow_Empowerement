-- ============================================================
-- Phase C: POS Concurrency Hardening
-- ============================================================

-- 1) Idempotency key column on pos_transactions
ALTER TABLE public.pos_transactions
  ADD COLUMN IF NOT EXISTS idempotency_key text;

-- Unique per register: same offline replay can't create duplicates
CREATE UNIQUE INDEX IF NOT EXISTS pos_transactions_idempotency_key_uidx
  ON public.pos_transactions (organization_id, business_id, register_id, idempotency_key)
  WHERE idempotency_key IS NOT NULL;

-- 2) Replace process_pos_transaction with idempotency + row-lock semantics
...
DECLARE
  ...
  v_product_name text;
BEGIN
  -- Idempotency short-circuit (replay safety)
  ...
  -- Warehouse: shift-locked → branch-default. NO business-wide fallback.
  ...
  -- ============================================================
  -- ROW-LEVEL STOCK LOCK PASS
  -- For each tracked product, lock the inventory row and verify
  -- sufficient available quantity (on_hand - reservations) BEFORE
  -- inserting the transaction. Two concurrent cashiers serialize here.
  -- Skipped entirely for returns (which add stock).
  -- ============================================================
  IF p_transaction_type <> 'return' AND v_default_warehouse_id IS NOT NULL THEN
    ...
    LOOP
      ...
      -- LOCK the inventory row (pessimistic). Other concurrent
      -- transactions touching the same (product, warehouse) wait here.
      ...
      -- Active reservations on this register's open carts (excluding this register's own current cart)
  ...
  INSERT INTO public.pos_transactions (
    ...
    status, completed_at, created_at, idempotency_key
  ) VALUES (
    ...
    'completed', now(), now(), p_idempotency_key
