...
DECLARE
  ...
  FUNCTION_KEY_CASH constant text := 'cash';
BEGIN
  ...
  -- Idempotency: the posting engine dedupes too, but return early for clarity.
  ...
  -- Debit side: expense override -> category mapping -> operating_expenses default.
  ...
  -- Credit side: explicit payment account, else per payment-method default.
...
GRANT EXECUTE ON FUNCTION public.post_expense_gl(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.post_expense_gl(uuid) TO service_role;