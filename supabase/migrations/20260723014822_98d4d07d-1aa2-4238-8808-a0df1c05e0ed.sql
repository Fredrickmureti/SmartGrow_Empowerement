...
-- ============================================================
-- 1. Pack extension registry
-- ============================================================
CREATE TABLE IF NOT EXISTS public.payroll_certificate_template_extensions (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  base_template_code TEXT NOT NULL,
  pack_code TEXT NOT NULL,
  region TEXT NOT NULL CHECK (region IN ('after_ytd','after_signature','appendix')),
  ...
  UNIQUE (base_template_code, pack_code, region, version)
);

GRANT SELECT ON public.payroll_certificate_template_extensions TO authenticated;
GRANT ALL ON public.payroll_certificate_template_extensions TO service_role;

ALTER TABLE public.payroll_certificate_template_extensions ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Anyone authenticated can read certificate template extensions"
  ON public.payroll_certificate_template_extensions
  FOR SELECT
  TO authenticated
  USING (true);

CREATE POLICY "Only service_role writes extensions"
  ON public.payroll_certificate_template_extensions
  FOR ALL
  TO service_role
  USING (true)
...
CREATE OR REPLACE FUNCTION public.set_updated_at_certificate_template_extensions()
RETURNS TRIGGER LANGUAGE plpgsql SET search_path = public AS $$
BEGIN NEW.updated_at := now(); RETURN NEW; END; $$;

DROP TRIGGER IF EXISTS trg_certificate_template_extensions_updated_at
  ON public.payroll_certificate_template_extensions;
CREATE TRIGGER trg_certificate_template_extensions_updated_at
  BEFORE UPDATE ON public.payroll_certificate_template_extensions
  FOR EACH ROW EXECUTE FUNCTION public.set_updated_at_certificate_template_extensions();

CREATE INDEX IF NOT EXISTS ix_cert_tmpl_ext_lookup
  ON public.payroll_certificate_template_extensions (base_template_code, pack_code, region);

-- ============================================================
-- 2. Republish ANNUAL_EARNINGS_STATEMENT (v2) — country-neutral,
--    richer DTO bindings, provenance footer, extension_regions.
-- ============================================================
...
SET body = $json$
{
  ...
  "paper_format": {
    ...
    "margin_top": 18, "margin_right": 14, "margin_bottom": 14, "margin_left": 14,
    "header_height": 20, "footer_height": 12
  },
  "page_master": {
    ...
    "footer": [
      { "type":"rich_text","align":"center",
        "paragraphs":[[
          ...
          {"text":{"kind":"binding","path":"provenance.content_hash_short","fallback":"—"}}
        ]] }
    ]
  },
  "document": [
    { "type":"heading","level":3,"align":"left",
      "text":{"kind":"binding","path":"period.label","format":"text","fallback":"Period"} },
    { "type":"identity_strip",
      ...
      "left":[
        ...
        {"type":"key_value","label":{"kind":"literal","value":"Legal name"},"value":{"kind":"binding","path":"employer.legal_name","fallback":"—"}},
        {"type":"key_value","label":{"kind":"literal","value":"Address"},"value":{"kind":"binding","path":"employer.registered_address","fallback":"—"}},
        {"type":"key_value","label":{"kind":"literal","value":"Contact"},"value":{"kind":"binding","path":"employer.contact","fallback":"—"}}
      ],
      "right":[
        ...
        {"type":"key_value","label":{"kind":"literal","value":"Department"},"value":{"kind":"binding","path":"employee.department","fallback":"—"}},
        {"type":"key_value","label":{"kind":"literal","value":"Position"},"value":{"kind":"binding","path":"employee.position","fallback":"—"}},
        {"type":"key_value","label":{"kind":"literal","value":"Employment period"},"value":{"kind":"binding","path":"employee.employment_period.label","fallback":"—"}}
      ]
    },
    ...
    { "type":"matrix",
      ...
      "footer":{
        ...
        "sum_columns":["gross","benefits","taxable","statutory_employee","statutory_employer","other_deductions","reliefs","net"]
      }
    },
    { "type":"spacer","size_mm":3 },
    { "type":"section",
      ...
      "children":[
        ...
        {"type":"key_value","label":{"kind":"literal","value":"Total Employer Contributions"},"value":{"kind":"binding","path":"ytd.statutory_employer","format":"currency"}},
    ...
    { "type":"signature_strip",
      "slots":[
        {"caption":{"kind":"binding","path":"issuer.name","fallback":"Authorised Signatory"},
         "sub_caption":{"kind":"binding","path":"issuer.title","fallback":"Payroll Office"}}
      ]
    },
