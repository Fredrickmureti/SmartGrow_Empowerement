-- Physical count posting is authoritative: allow it to drive stock to the
-- counted value even when the computed availability would go negative.
...
-- Restore the physical_count reference on the posting movement so it is exempt
-- from its own freeze; keep the signed 'adjustment' movement_type for correct
-- balance + cost-layer maintenance.
...
BEGIN
  ...
  LOOP
    ...
    -- Single signed adjustment movement, referenced to the physical count so it
    -- is exempt from its own freeze; the count is authoritative so shortages are
    -- allowed below zero by validate_stock_movement_quantity.
