...
-- T9: POS Transaction Engine — Idempotency Response Cache

CREATE TABLE IF NOT EXISTS public.pos_transaction_idempotency (
...
CREATE INDEX IF NOT EXISTS idx_pos_txn_idem_expires
  ON public.pos_transaction_idempotency(expires_at);
CREATE INDEX IF NOT EXISTS idx_pos_txn_idem_branch
  ON public.pos_transaction_idempotency(branch_id, created_at DESC);
...
DROP POLICY IF EXISTS pos_txn_idem_read ON public.pos_transaction_idempotency;
...
COMMENT ON TABLE public.pos_transaction_idempotency IS
  'ADR T9 · POS response-cache. process_pos_transaction stores the full JSONB response keyed by idempotency_key; retries return the cached body verbatim. 24h TTL — cleanup_pos_transaction_idempotency() reaps expired rows.';
...
$$;
GRANT EXECUTE ON FUNCTION public.cleanup_pos_transaction_idempotency() TO service_role;
...
BEGIN
  -- T9 entry: return the full cached JSONB response on retry.
  IF p_idempotency_key IS NOT NULL THEN
    ...
    -- Legacy fallback: pre-T9 transactions committed before the cache existed.
  ...
  -- T9 exit: cache the full response so retries return the exact body.
  IF p_idempotency_key IS NOT NULL THEN
