-- Salesperson Performance: single canonical server projection.
...
BEGIN
  ...
  WITH scoped_invoices AS (
    SELECT inv.id,
           inv.salesperson_id,
           inv.issue_date,
           (inv.total * COALESCE(NULLIF(inv.exchange_rate, 0), 1))::numeric AS base_total,
           COALESCE(NULLIF(inv.currency, ''), biz.base_currency, 'USD') AS currency,
           COALESCE(biz.base_currency, 'USD') AS base_currency
  ...
  revenue AS (
    SELECT si.salesperson_id,
           SUM(si.base_total)                                     AS gross_invoiced,
  ...
  credits AS (
    SELECT si.salesperson_id,
           SUM(cn.total * COALESCE(NULLIF(inv.exchange_rate, 0), 1))::numeric AS credit_value
  ...
  orders AS (
    SELECT so.salesperson_id,
           ...
           SUM(so.total * COALESCE(NULLIF(so.exchange_rate, 0), 1))::numeric AS orders_value
  ...
  collections AS (
    SELECT si.salesperson_id,
           SUM(pa.amount * COALESCE(NULLIF(inv.exchange_rate, 0), 1))::numeric AS cash_collected
...
GRANT EXECUTE ON FUNCTION public.get_salesperson_performance(uuid, uuid, uuid, date, date) TO authenticated;
...
BEGIN
  ...
  IF p_metric = 'revenue' THEN
    ...
    SELECT inv.id, 'invoice'::text, inv.invoice_number, inv.issue_date, inv.contact_id,
           ct.name::text,
           (inv.total * COALESCE(NULLIF(inv.exchange_rate, 0), 1))::numeric,
           inv.status::text
      FROM public.invoices inv
  ...
  ELSIF p_metric = 'credit' THEN
    ...
    SELECT cn.id, 'credit_note'::text, cn.credit_note_number, cn.issue_date, cn.contact_id,
           ct.name::text,
           (cn.total * COALESCE(NULLIF(inv.exchange_rate, 0), 1))::numeric,
           cn.status::text
      FROM public.credit_notes cn
  ...
  ELSIF p_metric = 'cash' THEN
    ...
    SELECT pmt.id, 'payment'::text, pmt.receipt_number, pmt.payment_date, pmt.contact_id,
           ct.name::text,
           (pa.amount * COALESCE(NULLIF(inv.exchange_rate, 0), 1))::numeric,
  ...
  ELSIF p_metric = 'orders' THEN
    ...
    SELECT so.id, 'sales_order'::text, so.so_number, so.order_date, so.contact_id,
           ct.name::text,
           (so.total * COALESCE(NULLIF(so.exchange_rate, 0), 1))::numeric,
           so.status
      FROM public.sales_orders so
...
GRANT EXECUTE ON FUNCTION public.get_salesperson_performance_documents(uuid, uuid, text, uuid, uuid, date, date) TO authenticated;