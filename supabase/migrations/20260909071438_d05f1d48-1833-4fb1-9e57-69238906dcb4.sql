CREATE OR REPLACE FUNCTION public.mf_collect_group_admission_fees(p_group_id uuid, p_collected_on date DEFAULT NULL::date, p_method text DEFAULT 'cash'::text, p_reference text DEFAULT NULL::text, p_notes text DEFAULT NULL::text, p_lines jsonb DEFAULT '[]'::jsonb, p_client_request_id text DEFAULT NULL::text)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  SELECT organization_id INTO v_org FROM public.businesses WHERE id = g.business_id;
  ...
  SELECT COALESCE(NULLIF(btrim(pol.admission_fee_currency), ''),
                  (SELECT b.base_currency FROM public.businesses b WHERE b.id = g.business_id))
    INTO v_currency
