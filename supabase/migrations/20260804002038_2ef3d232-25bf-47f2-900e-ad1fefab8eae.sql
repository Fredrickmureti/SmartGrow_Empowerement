...
BEGIN
  ...
  -- Devices: only ones that have actually connected before. A device
  -- claiming "online" with a dead heartbeat is reported as stale, not offline.
  FOR r IN
    SELECT d.id, d.display_name, d.role, d.status, d.last_error, d.last_seen_at,
           (d.status IN ('offline','error')) AS hard_down, w.warehouse_id
  ...
  LOOP
    PERFORM public.wms_raise_exception(
      ...
      p_kind => CASE
                ...
                END,
      p_reason => CASE
        WHEN r.hard_down THEN format('%s (%s) reported %s%s', COALESCE(r.display_name,'Device'),
                                     r.role, r.status, COALESCE(' — ' || NULLIF(r.last_error,''), ''))
        ELSE format('%s (%s) has not reported in since %s', COALESCE(r.display_name,'Device'),
                    r.role, to_char(r.last_seen_at, 'YYYY-MM-DD HH24:MI'))
      END,
      p_aggregate_type => 'device_assignment', p_aggregate_id => r.id,
      p_severity => CASE WHEN r.hard_down THEN 3::smallint ELSE 2::smallint END,
      p_source_system => 'hardware',
      p_details => jsonb_build_object('device_role', r.role, 'status', r.status,
                                      'last_error', r.last_error, 'last_seen_at', r.last_seen_at,
                                      'detection', CASE WHEN r.hard_down THEN 'reported_down' ELSE 'stale_heartbeat' END),
