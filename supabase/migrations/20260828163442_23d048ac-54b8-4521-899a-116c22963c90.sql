-- Previous attempt missed two cases: a no-op UPDATE (book already set, COALESCE
-- writes the same value) and a concurrent updated_at touch. Ignore updated_at
-- when comparing, and treat an unchanged row as allowed.
...
BEGIN
  ...
  -- Book stamp / no-op: a posted entry may be filed into a journal book when it
  -- had none, and an UPDATE that changes nothing material is not a change at
  -- all. updated_at is bookkeeping metadata and is ignored in the comparison.
