...
BEGIN
  ...
  -- Per-org override (app scope) wins. Treat override_value as boolean
  -- (jsonb true/false). Only honor active, non-expired overrides.
...
BEGIN
  ...
  -- 2. Direct plan_feature_access entry
  ...
  -- 4. Resolve feature → app via app_included_features
  FOR v_app_id IN
    SELECT app_id
