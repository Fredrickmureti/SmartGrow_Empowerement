-- Fix UUID/TEXT type mismatch in record_payment_atomic
-- The journal_entries.source_id column is UUID but was receiving TEXT values
...
BEGIN
  ...
  -- 4. Update invoice (cast text status to invoice_status enum)
  ...
  IF _cash_account_id IS NOT NULL AND _receivable_account_id IS NOT NULL THEN
    -- FIX: Compare UUID to UUID (not UUID to TEXT)
    SELECT id INTO v_je_id
      FROM journal_entries
     WHERE organization_id = _org_id
       AND source_type = 'payment'
       AND source_id = v_payment_id  -- FIXED: removed ::TEXT cast
       AND status != 'voided'
     LIMIT 1;

    IF v_je_id IS NULL THEN
      -- FIX: Insert UUID directly (not TEXT)
      INSERT INTO journal_entries (
        ...
        'payment', v_payment_id, 'posted', _created_by  -- FIXED: removed ::TEXT cast
