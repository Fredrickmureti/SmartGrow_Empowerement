...
BEGIN
  ...
  RETURN jsonb_build_object('task_id', p_task_id, 'location_id', p_location_id, 'override', v_override);
END; $function$;

REVOKE ALL ON FUNCTION public.wms_reassign_putaway_task(uuid, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.wms_reassign_putaway_task(uuid, uuid, text) TO authenticated, service_role;
...
REVOKE ALL ON FUNCTION public.wms_split_putaway_task(uuid, numeric, uuid, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.wms_split_putaway_task(uuid, numeric, uuid, text) TO authenticated, service_role;

-- Report an exception from the putaway floor ---------------------------
CREATE OR REPLACE FUNCTION public.wms_report_putaway_exception(
  p_task_id uuid,
...
LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public'
AS $function$
DECLARE v_task record; v_exc uuid;
BEGIN
  SELECT * INTO v_task FROM public.wms_tasks WHERE id = p_task_id;
  IF v_task.id IS NULL THEN RAISE EXCEPTION 'task % not found', p_task_id; END IF;
  IF NOT user_can_access_business(auth.uid(), v_task.business_id) THEN RAISE EXCEPTION 'access denied'; END IF;
  ...
  v_exc := public.wms_raise_exception(
    p_warehouse_id => v_task.warehouse_id,
    p_kind => p_kind,
    p_reason => p_reason,
    p_aggregate_type => 'wms_task',
    p_aggregate_id => p_task_id,
    p_task_id => p_task_id,
    p_lpn_id => v_task.lpn_id,
    p_details => COALESCE(p_details,'{}'::jsonb) || jsonb_build_object(
      'task_type', 'putaway',
      'product_id', v_task.product_id,
      'lot_number', v_task.lot_number,
      'quantity', v_task.quantity,
      'source_location_id', v_task.source_location_id,
      'destination_location_id', v_task.destination_location_id),
    p_idempotency_key => 'wms.putaway.exception:' || p_task_id::text || ':' || p_kind::text,
    p_source_system => 'putaway'
  );

  UPDATE public.wms_tasks
     SET state = 'exception',
         metadata = COALESCE(metadata,'{}'::jsonb) || jsonb_build_object(
           'exception_id', v_exc, 'exception_kind', p_kind::text, 'exception_reason', p_reason)
   WHERE id = p_task_id;

  RETURN jsonb_build_object('task_id', p_task_id, 'exception_id', v_exc);
END; $function$;

REVOKE ALL ON FUNCTION public.wms_report_putaway_exception(uuid, public.wms_exception_kind, text, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.wms_report_putaway_exception(uuid, public.wms_exception_kind, text, jsonb) TO authenticated, service_role;
