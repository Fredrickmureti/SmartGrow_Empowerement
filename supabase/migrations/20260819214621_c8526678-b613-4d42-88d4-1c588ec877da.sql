-- Phase 2 follow-up: allow trusted server contexts (migrations, pgTAP
-- self-tests, service_role jobs) to exercise the reporting RPCs.
-- _assert_org_member() raises 'Not authenticated' when auth.uid() is NULL,
-- which blocked self-tests. Authorization for real API callers is unchanged:
-- org membership + business access + branch access are all still enforced.
