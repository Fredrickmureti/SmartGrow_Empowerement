CREATE OR REPLACE FUNCTION public.upsert_notification_alert_settings(_organization_id uuid, _business_id uuid, _settings jsonb)
 RETURNS notification_alert_settings
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Authorization: caller must be an admin/owner/super_admin of this org.
  -- Uses the canonical user_roles helper (the org membership table in this
  -- project is `user_roles`, not the non-existent `organization_members`).
  IF NOT public.is_org_admin_or_owner(v_caller, _organization_id) THEN
