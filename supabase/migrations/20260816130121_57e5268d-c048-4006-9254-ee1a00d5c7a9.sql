-- 1. Movement type vocabulary: align the constraint with what the canonical
--    engines actually emit. The previous list omitted transfer_out/transfer_in
--    (emitted by approve_stock_transfer_atomic / complete_stock_transfer_atomic,
--    consume_lots_atomic, wms_lpn_dispatch, wms_lpn_move, wms_split_putaway_task),
--    adjustment_in/adjustment_out, opening_stock, customer_return and
--    vendor_return, so every inter-warehouse transfer failed at approval.
...
ALTER TABLE public.stock_movements
  ...
  CHECK (movement_type = ANY (ARRAY[
    'purchase','receipt','opening','opening_stock','migration',
    'sale','pos_sale','pos_return','delivery',
    'adjustment','adjustment_in','adjustment_out',
    'return_in','return_out','customer_return','vendor_return',
    'transfer','transfer_in','transfer_out',
    'scrap','count','landed_cost'
  ]::text[]));

-- 2. Cost layer maintenance: an inbound transfer leg carries no unit_cost (the
--    transfer engines do not stamp one), which created zero-cost layers at the
--    destination and destroyed inventory value on every transfer. The layer now
--    inherits the weighted cost of the parent layers it descends from, using the
--    lineage rows this same trigger writes.
