...
BEGIN
  ...
  -- Totals of the reversal are the mirror of the original's lines. They are
  -- computed up front so the header can be inserted already balanced: a posted
  -- entry may not be UPDATEd afterwards (immutability trigger), which is why
  -- the line-level recompute trigger is suppressed below — exactly the shape
  -- post_journal_entry_atomic uses.
