-- Shared read-access rule for AP reporting: org members, or trusted
-- server-side (service_role) callers such as document generation.
CREATE OR REPLACE FUNCTION public.finance_can_read_org(_org_id uuid)
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT
    ...
    OR (auth.uid() IS NOT NULL AND public.is_org_member(auth.uid(), _org_id));
$$;

REVOKE ALL ON FUNCTION public.finance_can_read_org(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.finance_can_read_org(uuid) TO authenticated, service_role;
