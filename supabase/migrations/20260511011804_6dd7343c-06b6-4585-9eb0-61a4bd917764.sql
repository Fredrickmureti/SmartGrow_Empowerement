...
DROP FUNCTION IF EXISTS public.process_pos_return(uuid, uuid, uuid, uuid, jsonb, text, text, uuid);
...
BEGIN
  ...
  -- Stage 4 closeout: cross-tender detection.
  ...
  IF v_is_cross_tender THEN
    IF p_override_id IS NULL THEN
      RETURN jsonb_build_object('success', false, 'error', 'cross_tender_requires_override',
        'details', format('Refund tender "%s" was not used on the original sale; manager approval required', v_refund_tender));
    END IF;
    ...
    SELECT EXISTS (
      SELECT 1 FROM public.pos_manager_overrides
       WHERE id = p_override_id
         AND organization_id = p_organization_id
         AND override_type   = 'cross_tender_refund'
         AND (transaction_id IS NULL OR transaction_id = p_original_transaction_id)
