...
BEGIN
  ...
  WITH agg AS (
    ...
    WHERE pl.payroll_run_id = p_run_id
      ...
      -- Advance recovery relieves the advance receivable (ADR 0124); it is not
      -- a payable, so it must not demand an `advance_recovery_payable` mapping.
      AND lower(pl.rule_code) NOT LIKE 'advance_recovery%'
      AND COALESCE(pl.source->>'source', '') <> 'custom_deduction'
      AND COALESCE(pl.source->>'kind', '')   <> 'loan_repayment'
      AND COALESCE(pl.source->>'kind', '')   <> 'advance_recovery'
  ...
  needed AS (
    ...
    UNION ALL
    SELECT 'employee_advance_receivable',
           'Employee Advance Receivable',
           NULL, 'advance_receivable', 'asset'
    FROM advances_used
  ),
  needed_dedup AS (
  ...
  SELECT
    ...
    CASE
      ...
      -- Advances fall back to the loan receivable account, mirroring
      -- `disburse_employee_advance`, so either mapping satisfies the run.
      WHEN n.setting_key = 'employee_advance_receivable'
        THEN EXISTS (SELECT 1 FROM effective_mappings em
                      WHERE em.setting_key IN ('employee_advance_receivable','loan_receivable')
                        AND em.account_id IS NOT NULL)
