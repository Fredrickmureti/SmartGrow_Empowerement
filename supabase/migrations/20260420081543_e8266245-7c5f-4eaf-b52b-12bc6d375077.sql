-- 1. Add localization_status column to organizations
ALTER TABLE public.organizations
  ADD COLUMN IF NOT EXISTS localization_status text NOT NULL DEFAULT 'pending'
    CHECK (localization_status IN ('pending', 'installed', 'generic', 'failed'));

COMMENT ON COLUMN public.organizations.localization_status IS
  'Tracks country localization pack install state: pending|installed|generic|failed';

-- 2. Add "owner" to employment_type enum (if not already present)
DO $$
BEGIN
  IF NOT EXISTS (
...
EXCEPTION WHEN undefined_object THEN
  -- employment_type may be plain text on this DB; ignore
  NULL;
END$$;

-- 3. Replace complete_onboarding with the captured live state + audit fixes
...
BEGIN
  ...
  -- Determine initial localization status: 'generic' if no pack exists for this country, else 'pending'
  SELECT EXISTS (
  ...
  -- Provision the founding company
  ...
  -- Founder employee — basic_salary NULL (not 0), employment_type 'owner'
  ...
  EXCEPTION WHEN invalid_text_representation OR undefined_object THEN
    -- Fallback for older enum without 'owner'
    INSERT INTO public.employees (
      organization_id, business_id, employee_number,
      first_name, last_name, email, user_id,
      position, hire_date, employment_type, basic_salary
