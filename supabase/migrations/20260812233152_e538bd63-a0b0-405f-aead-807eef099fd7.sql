...
-- FX exposure reporting (ADR 0136): a read-only projection over the SAME
-- open-balance scope as revalue_fx_balances / fx_revaluation_readiness, so the
-- exposure report and the revaluation run can never disagree.

CREATE OR REPLACE FUNCTION public.fx_exposure_by_currency(
  _business_id uuid,
...
DECLARE
  ...
  _r record;
BEGIN
  SELECT b.organization_id, upper(COALESCE(b.base_currency, b.default_currency))
    INTO _org, _base
...
REVOKE ALL ON FUNCTION public.fx_exposure_by_currency(uuid, date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.fx_exposure_by_currency(uuid, date) TO authenticated;
COMMENT ON FUNCTION public.fx_exposure_by_currency(uuid, date) IS
  'ADR 0136 — read-only FX exposure projection. Same open-balance scope as revalue_fx_balances; rates come from resolve_exchange_rate/describe_exchange_rate only. Never values a missing rate at 1:1.';


CREATE OR REPLACE FUNCTION public.fx_exposure_open_items(
  _business_id uuid,
  _currency text,
  _as_of date DEFAULT CURRENT_DATE
)
...
REVOKE ALL ON FUNCTION public.fx_exposure_open_items(uuid, text, date) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.fx_exposure_open_items(uuid, text, date) TO authenticated;
COMMENT ON FUNCTION public.fx_exposure_open_items(uuid, text, date) IS
  'ADR 0136 — document-level drill-down behind fx_exposure_by_currency. Read-only; posts nothing.';
