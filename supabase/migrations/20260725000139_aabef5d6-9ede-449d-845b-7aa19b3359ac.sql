...
DROP FUNCTION IF EXISTS public.garnishment_transition(uuid, text, text, text, text, jsonb);
...
BEGIN
  ...
  -- R6: documentary-evidence guard for terminal transitions.
  v_required_doc_kind := CASE p_action
    WHEN 'release'               THEN 'release'
    WHEN 'mark_satisfied'        THEN 'satisfaction'
    WHEN 'terminate_unsatisfied' THEN 'termination'
    ELSE NULL
  END;

  IF v_required_doc_kind IS NOT NULL THEN
    IF p_evidence_document_id IS NULL THEN
      RAISE EXCEPTION 'GARNISHMENT_EVIDENCE_REQUIRED: action % requires a % document (upload to legal_order_documents first)',
        p_action, v_required_doc_kind
        USING ERRCODE = '22023', HINT = 'GOV_EVIDENCE_REQUIRED';
    END IF;

    SELECT garnishment_id, document_kind
      INTO v_doc_garn, v_doc_kind
      FROM public.legal_order_documents
     WHERE id = p_evidence_document_id;

    IF v_doc_garn IS NULL THEN
      RAISE EXCEPTION 'GARNISHMENT_EVIDENCE_NOT_FOUND: legal_order_documents % does not exist', p_evidence_document_id
        USING ERRCODE = 'P0002';
    END IF;

    IF v_doc_garn <> v_row.id THEN
      RAISE EXCEPTION 'GARNISHMENT_EVIDENCE_MISMATCH: document % belongs to a different legal order', p_evidence_document_id
        USING ERRCODE = '22023';
    END IF;

    IF v_doc_kind <> v_required_doc_kind THEN
      RAISE EXCEPTION 'GARNISHMENT_EVIDENCE_WRONG_KIND: action % needs a % document, got %',
        p_action, v_required_doc_kind, v_doc_kind
        USING ERRCODE = '22023';
    END IF;
  END IF;
...
COMMENT ON FUNCTION public.garnishment_transition(uuid, text, text, text, text, jsonb, uuid) IS
  'ADR-0093 R6: legal-order FSM. Terminal transitions (release, mark_satisfied, terminate_unsatisfied) require a matching legal_order_documents row supplied via p_evidence_document_id; non-terminal transitions are unchanged.';
