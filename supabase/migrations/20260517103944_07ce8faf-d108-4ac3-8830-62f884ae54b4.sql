-- G1: Auto-DN from invoice should default received_by_contact_id to the
-- invoice customer (parity with record_partial_delivery_atomic, which
-- already stamps v_dn.contact_id as the receiver). Without this, the
-- dialog's "Received By" row stays blank for a confirmed-and-completed
-- auto-DN even though the customer of record is known.
...
-- Backfill: stamp received_by_contact_id on any existing auto-DNs that
-- were created by the previous version of this function and have no
-- receiver set yet. Safe because we only touch rows where the column
-- is currently NULL and a source invoice is known.
UPDATE public.delivery_notes dn
   SET received_by_contact_id = i.contact_id
  FROM public.invoices i
 WHERE dn.received_by_contact_id IS NULL
   AND dn.source_invoice_id IS NOT NULL
   AND i.id = dn.source_invoice_id
