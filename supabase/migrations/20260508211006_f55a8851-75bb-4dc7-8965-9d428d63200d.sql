-- ============================================================================
-- Phase 1: canonical, run-aware payroll GL mapping resolver
-- ============================================================================

-- 1. Run-specific required mapping keys, derived from payslip_lines.
--    payslip_lines.rule_code is the country-agnostic identifier for every
--    earning / deduction / employer contribution. We do NOT read
--    payslips.deductions_detail (human-label JSON) anymore — that was the
--    source of the "missing PAYE (Pay As You Earn)_payable" mismatch with
--    payroll_gl_readiness which already uses normalized rule_code keys.
...
BEGIN
  ...
  WITH agg AS (
    ...
    WHERE pl.payroll_run_id = p_run_id
      ...
      AND pl.category NOT IN ('earning')
...
GRANT EXECUTE ON FUNCTION public.payroll_required_gl_mappings_for_run(uuid)
  TO authenticated, service_role;

-- 2. Rewrite validate_payroll_run_mappings to use the canonical resolver.
--    Returns the same { ok, missing_keys, required_keys, existing_keys, summary }
--    shape the client already consumes, but now with the SAME truth as
--    payroll_gl_readiness and post-payroll-gl.
...
BEGIN
  ...
  IF auth.uid() IS NOT NULL
     AND NOT public.is_org_member(auth.uid(), v_run.organization_id) THEN
    RAISE EXCEPTION 'Not authorized to validate this payroll run' USING ERRCODE = '42501';
  END IF;

  SELECT
    COALESCE(array_agg(setting_key ORDER BY setting_key), ARRAY[]::text[]),
    COALESCE(array_agg(setting_key ORDER BY setting_key) FILTER (WHERE is_mapped),     ARRAY[]::text[]),
    COALESCE(array_agg(setting_key ORDER BY setting_key) FILTER (WHERE NOT is_mapped), ARRAY[]::text[])
  INTO v_required_keys, v_existing_keys, v_missing_keys
  FROM public.payroll_required_gl_mappings_for_run(p_run_id);

  RETURN jsonb_build_object(
    'ok', cardinality(v_missing_keys) = 0,
    'missing_keys', v_missing_keys,
    'required_keys', v_required_keys,
    'existing_keys', v_existing_keys,
    'summary', CASE
      WHEN cardinality(v_missing_keys) = 0
        THEN 'All required account mappings are configured.'
      ELSE format(
        '%s mapping(s) missing — configure them in Payroll → Configuration → GL Account Mapping.',
        cardinality(v_missing_keys)
      )
    END
  );
END;
...
GRANT EXECUTE ON FUNCTION public.validate_payroll_run_mappings(uuid)
  TO authenticated, service_role;