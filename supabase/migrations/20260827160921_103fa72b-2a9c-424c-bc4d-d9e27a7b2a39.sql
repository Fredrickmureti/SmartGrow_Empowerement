CREATE OR REPLACE FUNCTION public.consolidation_diagnose_eliminations(
  _group_id uuid,
  _date_from date,
  _date_to date
)
RETURNS TABLE (
  ...
  business_a_id uuid,
  business_a_name text,
  business_a_currency text,
  business_b_id uuid,
  business_b_name text,
  business_b_currency text,
  presentation_currency text,
  difference_signed numeric,
  difference_amount numeric,
  effective_tolerance numeric,
  effective_policy text,
  rule_exists boolean,
  is_cross_currency boolean,
  cause text,
  would_refuse boolean,
  suggested_tolerance numeric,
  remedies text[],
  message text
)
LANGUAGE plpgsql
...
BEGIN
  ...
  -- The diagnosis reveals pair-level intra-group disagreements, which is exactly
  -- the sensitivity of generating the eliminations themselves, so it is gated on
  -- the same authority.
  ...
  -- The engine reads the same figures through the same function. When that read
  -- itself refuses -- an unreportable scope, an entry tagged to two companies,
  -- an account the consolidated trial balance does not carry -- the diagnosis
  -- reports the refusal as a finding instead of throwing, so the caller still
  -- learns where to go.
  BEGIN
  ...
  LOOP
    ...
    -- A class switched off produces nothing, so it can refuse nothing.
    ...
    LOOP
      ...
      IF abs(v_diff) <= v_tol THEN
        v_cause := 'within_tolerance';
        v_refuse := false;
        v_remedies := ARRAY[]::text[];
        v_message := format(
          'The two sides of the %s position between %s and %s differ by %s %s, which is within the tolerance of %s %s in force, so the run proceeds and the gap is left unrecorded.',
      ...
      ELSIF v_policy = 'post_to_cta' AND NOT v_cross THEN
        -- The engine refuses this: both members already report in the group's
        -- currency, so no part of the gap can be a translation effect.
      ...
      ELSIF v_policy <> 'refuse' THEN
        -- A policy that can absorb the gap: the run succeeds and discloses it.
...
REVOKE ALL ON FUNCTION public.consolidation_diagnose_eliminations(uuid, date, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.consolidation_diagnose_eliminations(uuid, date, date) TO authenticated;
GRANT EXECUTE ON FUNCTION public.consolidation_diagnose_eliminations(uuid, date, date) TO service_role;

COMMENT ON FUNCTION public.consolidation_diagnose_eliminations(uuid, date, date) IS
'Read-only preflight for consolidation_generate_eliminations. Reports, per elimination class and per pair of members, the residual between the two declared sides, the tolerance and difference policy in force, whether the residual is a translation effect or a genuine disagreement, and the remedy codes the engine would accept. Reads the same intercompany flows the engine reads, so preflight and run cannot disagree. Never writes.';