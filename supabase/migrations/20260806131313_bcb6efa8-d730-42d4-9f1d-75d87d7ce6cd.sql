...
CREATE OR REPLACE FUNCTION public.confirm_vendor_credit_note_atomic(_vcn_id uuid, _user_id uuid)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  v_je_number := public.generate_next_je_number(v_vcn.organization_id, v_vcn.business_id);

  v_je_id := public.post_journal_entry_atomic(
    v_vcn.organization_id, v_vcn.business_id,
    v_je_number, v_vcn.credit_date,
    v_vcn.credit_note_number,
    'Vendor credit note ' || v_vcn.credit_note_number,
    'vendor_credit_note', v_vcn.id, _user_id, false, false,
    jsonb_build_array(
      jsonb_build_object('account_id', v_ap_account, 'debit', v_vcn.total, 'credit', 0,
        'description', 'VCN ' || v_vcn.credit_note_number || ' — AP reduction',
        'contact_id', v_vcn.vendor_id),
      jsonb_build_object('account_id', v_exp_account, 'debit', 0, 'credit', v_vcn.total,
        'description', 'VCN ' || v_vcn.credit_note_number || ' — expense credit')
    ),
    NULL, NULL, NULL, v_vcn.branch_id
  );
