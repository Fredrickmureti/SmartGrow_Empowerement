-- ============================================================================
-- Phase 2 correction — real column names for the related-document projection.
--
-- fiscal_transmissions keys documents as (source_doc_type, source_doc_id) with
-- state in ('queued','transmitting','succeeded','rejected','retry_scheduled',
-- 'failed','dead_letter','superseded'); only 'succeeded' means the tax
-- authority accepted it. delivery_notes has no invoice_id: a note either came
-- FROM an invoice (source_invoice_id) or SPAWNED one (spawned_invoice_id).
-- ============================================================================
...
BEGIN
  ...
  IF _document_type = 'invoice' THEN
    ...
    -- Money: live settlement, through the allocation ledger (ADR 0027).
    SELECT COALESCE(jsonb_agg(jsonb_build_object(
             'payment_id',       p.id,
    ...
    -- Related documents. A delivery note either sources this invoice or was
    -- spawned by it; both directions bind goods movement to the document.
