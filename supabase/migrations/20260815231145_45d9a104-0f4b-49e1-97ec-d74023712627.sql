...
DROP FUNCTION IF EXISTS public.list_products_with_branch_stock(uuid, uuid, uuid, boolean, uuid);
...
RETURNS TABLE(
  id uuid, organization_id uuid, business_id uuid, type text, name text, description text, sku text,
  unit_price numeric, cost_price numeric, tax_rate numeric, tax_rate_id uuid, is_active boolean,
  image_url text, category_id uuid, track_inventory boolean, reorder_level numeric,
  ...
  packaging jsonb, on_hand numeric, reserved numeric, available numeric, branch_scope_label text,
  category_name text, tax_rate_name text, etims_tax_code text,
  is_weighted boolean, plu_code text
)
LANGUAGE plpgsql
...
BEGIN
  ...
  SELECT
    ...
    p.unit_price, p.cost_price, COALESCE(tr.rate, p.tax_rate), p.tax_rate_id, p.is_active, p.image_url, p.category_id,
...
REVOKE ALL ON FUNCTION public.list_products_with_branch_stock(uuid, uuid, uuid, boolean, uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.list_products_with_branch_stock(uuid, uuid, uuid, boolean, uuid) TO authenticated, service_role;