-- The ledger returns nominal (income/expense) openings as credits-minus-debits,
-- i.e. credit-positive, while the shared accounting kernel treats expense as
-- debit-normal. Normalise expense openings to the kernel's convention here so
-- closing balances and translation residuals are arithmetically consistent.
...
BEGIN
  ...
  SELECT g.organization_id INTO v_org FROM public.consolidation_groups g WHERE g.id = _group_id;
  ...
  LOOP
    ...
    normalised AS (
      SELECT c.account_id, a.code, a.name, a.account_type, c.is_nominal,
             ...
             (c.opening_balance <> 0 OR c.total_debit <> 0 OR c.total_credit <> 0) AS has_activity
        FROM combined c
    ...
    SELECT
      ...
      n.account_id, n.code, n.name, n.account_type, n.is_nominal,
      n.opening_balance, n.total_debit, n.total_credit,
      CASE WHEN n.debit_normal
           THEN n.opening_balance + n.total_debit - n.total_credit
           ELSE n.opening_balance + n.total_credit - n.total_debit END
    FROM normalised n
    WHERE n.has_activity;
  END LOOP;
END;
$function$;
