...
BEGIN
  ...
  -- One shared definition of an open monetary position (R5), now measured per
  -- counterparty so the adjustment lands on the same balance the intercompany
  -- matching compares.
  FOR _row IN
    SELECT p.account_id, p.contact_id, p.currency, p.foreign_balance, p.base_balance_old
  ...
  LOOP
    ...
    -- Unrealized FX belongs to the legal entity, never to a branch. The
    -- balance-side line keeps the counterparty of the position it adjusts.
    IF _delta > 0 THEN
      _lines := _lines
