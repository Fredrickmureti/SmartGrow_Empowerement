CREATE OR REPLACE FUNCTION public.process_pos_transaction(
  ...
  p_notes text DEFAULT NULL, p_cashier_id uuid DEFAULT NULL, p_created_by uuid DEFAULT NULL,
...
BEGIN
  ...
  IF p_transaction_type <> 'return' AND v_default_warehouse_id IS NOT NULL THEN
    ...
    LOOP
      ...
      -- FIX: was selecting from non-existent public.inventory(quantity_on_hand).
      -- The canonical per-warehouse stock table is public.warehouse_stock(quantity).
