...
BEGIN
  ...
  SELECT organization_id, base_currency INTO v_org, v_base
    FROM public.businesses WHERE id = ch.business_id;
  IF v_org IS NULL THEN
    RAISE EXCEPTION 'Institution % has no organization', ch.business_id;
  END IF;

  v_ccy := COALESCE(NULLIF(btrim(ch.currency_code), ''), v_base);
  ...
  IF v_ccy = v_base THEN
    v_rate := 1;
  ELSE
    v_rate := public.require_exchange_rate(v_org, ch.business_id, v_ccy, v_paid_on);
  END IF;
