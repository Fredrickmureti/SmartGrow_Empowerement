...
BEGIN
  ...
  IF v_lines_inserted > 0 THEN
    -- Transaction-local freeze override: the count's own reconciliation
    -- adjustment must be allowed to write stock_movements against the
    -- warehouse it froze. The GUC auto-resets at COMMIT/ROLLBACK so it
    -- cannot leak to other statements or sessions.
    BEGIN
