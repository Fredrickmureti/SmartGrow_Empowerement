-- Fix storage_used_mb in get_user_session_data to use real storage calculation
-- Previously hardcoded to 0, now calls get_org_storage_usage_mb()
...
BEGIN
  ...
  SELECT jsonb_agg(
    jsonb_build_object(
      ...
      -- FIX: Now uses get_org_storage_usage_mb() instead of hardcoded 0
      'usage_counters', jsonb_build_object(
        ...
        'storage_used_mb', COALESCE(public.get_org_storage_usage_mb(o.id), 0)
      ),
...
-- Add platform_logo_url setting if not exists
INSERT INTO platform_settings (setting_key, setting_value, description)
VALUES ('platform_logo_url', '', 'Platform logo URL for admin reports and branding')
