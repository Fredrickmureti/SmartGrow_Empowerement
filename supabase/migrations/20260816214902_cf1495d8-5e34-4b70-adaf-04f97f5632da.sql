...
BEGIN
  ...
  BEGIN
    INSERT INTO public.purchase_orders(organization_id, business_id, branch_id, vendor_id,
        po_number, status, order_date, currency, deliver_to_warehouse_id, created_by, subtotal, total)
    VALUES (v_org, v_biz, v_branch, v_vendor,
        'PO-SIM-' || to_char(clock_timestamp(),'YYYYMMDDHH24MISS'), 'approved', CURRENT_DATE,
        'KES', v_wh, v_uid, 240, 240) RETURNING id INTO v_po;
    INSERT INTO public.purchase_order_items(purchase_order_id, product_id, description,
        display_quantity, packaging_id, display_uom_id, unit_price, sort_order, tax_rate, tax_amount, line_total)
    VALUES (v_po, v_product, 'Sugar (simulation)', 2, v_pack, v_duom, 120, 1, 0, 0, 240) RETURNING id INTO v_poi;

    v_res := public.create_goods_receipt(v_biz, v_po,
      jsonb_build_array(jsonb_build_object('purchase_order_item_id', v_poi,
        'product_id', v_product, 'display_quantity', 2)), v_uid, v_wh, NULL, CURRENT_DATE);
    ...
    v_res := public.purchase_return_create(v_biz, v_vendor,
      jsonb_build_array(jsonb_build_object('goods_receipt_item_id', v_line.goods_receipt_item_id,
        'product_id', v_line.product_id, 'description', v_line.description,
        'display_quantity', 1, 'packaging_id', v_pack, 'display_uom_id', v_duom,
        'quantity', 50, 'return_reason', 'damaged')),
    ...
    PERFORM public.purchase_return_submit(v_pr, v_ver);
    SELECT row_version INTO v_ver FROM public.purchase_returns WHERE id=v_pr;
    PERFORM public.purchase_return_approve(v_pr, v_ver, 'Simulation approval');
    SELECT row_version INTO v_ver FROM public.purchase_returns WHERE id=v_pr;
    PERFORM public.purchase_return_dispatch(v_pr, v_ver, CURRENT_DATE, 'SIM-TRACK-1');
    SELECT row_version INTO v_ver FROM public.purchase_returns WHERE id=v_pr;
    PERFORM public.purchase_return_acknowledge(v_pr, v_ver, 'RMA-SIM-1', NULL);
    SELECT row_version INTO v_ver FROM public.purchase_returns WHERE id=v_pr;
