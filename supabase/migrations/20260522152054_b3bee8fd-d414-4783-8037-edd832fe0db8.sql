-- =====================================================================
-- Wave 9 — Restore canonical JE posting in approve_stock_adjustment_atomic
-- and add atomic product + opening-stock RPC.
-- =====================================================================

-- 1. approve_stock_adjustment_atomic: post via post_journal_entry_atomic
...
BEGIN
  ...
  -- Per-offset accumulator (kept as TEMP table — created fresh per txn via ON COMMIT DROP).
  ...
  IF v_total_value > 0 THEN
    -- Build balanced line array: per-offset lines + aggregated inventory line(s).
...
-- 2. Atomic product + opening-stock creation
...
BEGIN
  ...
  -- Insert the product (whitelist columns we accept from the payload).
  ...
  -- If opening items provided, validate and route through the adjustment engine.
  ...
  IF v_has_opening THEN
    -- All items must share a single warehouse (Odoo-style invariant) — same as client guard.
    SELECT count(DISTINCT (i->>'warehouse_id')) INTO v_wh_count
      FROM jsonb_array_elements(v_items_arr) AS i
     WHERE COALESCE((i->>'quantity_adjustment')::numeric, 0) > 0;
    IF v_wh_count <> 1 THEN
      RAISE EXCEPTION 'All opening-stock lines must use the same warehouse';
    END IF;

    SELECT (i->>'warehouse_id')::uuid INTO v_warehouse_id
      FROM jsonb_array_elements(v_items_arr) AS i
     WHERE COALESCE((i->>'quantity_adjustment')::numeric, 0) > 0
     LIMIT 1;

    SELECT business_id, COALESCE(is_in_transit,false), branch_id
      INTO v_wh_business, v_wh_in_transit, v_branch_id
      FROM public.warehouses
     WHERE id = v_warehouse_id;

    IF v_wh_business IS NULL THEN
      RAISE EXCEPTION 'Warehouse % not found', v_warehouse_id;
    END IF;
    IF v_wh_business <> v_biz_id THEN
      RAISE EXCEPTION 'Warehouse % belongs to a different company', v_warehouse_id;
    END IF;
    IF v_wh_in_transit THEN
      RAISE EXCEPTION 'Cannot use the in-transit warehouse for opening stock';
    END IF;
    IF v_branch_id IS NULL THEN
      RAISE EXCEPTION 'Cannot resolve branch for warehouse %', v_warehouse_id;
    END IF;
    ...
    IF NOT COALESCE((v_adj_result->>'success')::boolean, false) THEN
      -- Bubble the precise reason so the whole txn rolls back with context.
      RAISE EXCEPTION 'Opening stock failed: %', COALESCE(v_adj_result->>'error', 'unknown error');
    END IF;
  END IF;
  ...
  RETURN jsonb_build_object(
    'success', true,
    'product_id', v_product_id,
    'opening_stock', v_adj_result
  );
END;
$function$;

GRANT EXECUTE ON FUNCTION public.create_product_with_opening_stock_atomic(jsonb, jsonb, uuid)
