-- ─────────────────────────────────────────────────────────────────────
-- Settlement convergence — Phase C
-- 1. Employee advance disbursement posts through the canonical engine.
-- 2. Drop the redundant employee_loan_apply_repayment overload.
-- ─────────────────────────────────────────────────────────────────────
...
-- Canonical disbursement: state transition + GL posting in one place.
...
BEGIN
  ...
  IF v_adv.status = 'requested' THEN
  ...
  -- Idempotent replay: the engine dedups by (source_type, source_id), but we
  -- short-circuit so repeated clicks return the same proof cheaply.
  ...
  IF v_existing IS NOT NULL THEN
    UPDATE public.employee_advances
       SET journal_entry_id = v_existing,
           status = CASE WHEN status = 'approved' THEN 'disbursed'::advance_status ELSE status END,
  ...
  -- Debit: employee advance receivable (asset). Falls back to the loan
  -- receivable control account when no advance-specific mapping exists.
  ...
  -- Credit: the funding account the cash actually left.
  ...
  UPDATE public.employee_advances
     SET status = 'disbursed'::advance_status,
         disbursed_at = COALESCE(disbursed_at, now()),
...
-- Duplicate implementation removal: the 6-arg shim is a compatibility layer
-- over the full 8-arg signature. One implementation only.
DROP FUNCTION IF EXISTS public.employee_loan_apply_repayment(uuid, numeric, uuid, uuid, text, text);
