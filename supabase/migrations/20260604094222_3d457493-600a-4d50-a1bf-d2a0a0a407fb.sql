...
AS $function$
  ...
  items AS (
    SELECT jsonb_agg(jsonb_build_object(
      ...
      -- Multi-unit pack provenance (Phase B unification).
      'packaging_id',     ti.packaging_id,
      'display_uom_id',   ti.display_uom_id,
      'display_quantity', ti.display_quantity,
      'packaging_label',  pp.name,
      'qty_in_base_uom',  pp.qty_in_base_uom,
      'base_uom_label',   COALESCE(uom.code, uom.name, 'ea'),
      'uom_snapshot',     CASE
        WHEN pp.id IS NOT NULL AND COALESCE(pp.qty_in_base_uom, 1) > 1
          THEN pp.name || ' × ' || pp.qty_in_base_uom::text || ' ' || COALESCE(uom.code, uom.name, 'ea')
    ...
    LEFT JOIN public.product_packaging    pp  ON pp.id  = ti.packaging_id
    LEFT JOIN public.units_of_measure     uom ON uom.id = COALESCE(ti.display_uom_id, p.base_uom_id)
