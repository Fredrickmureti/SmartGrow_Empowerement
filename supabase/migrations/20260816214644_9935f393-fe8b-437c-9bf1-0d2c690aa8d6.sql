...
BEGIN
  ...
  BEGIN
    ...
    VALUES (v_po, v_product, 'Sugar (simulation)', 2, v_pack, v_duom, 120, 1, 0, 0, 240)
    ...
    v_res := public.create_goods_receipt(v_biz, v_po,
      jsonb_build_array(jsonb_build_object('purchase_order_item_id', v_poi,
        'product_id', v_product, 'display_quantity', 2)),
    ...
    IF (SELECT status FROM public.purchase_returns WHERE id=v_pr) IN ('dispatched','in_transit') THEN
      SELECT row_version INTO v_ver FROM public.purchase_returns WHERE id=v_pr;
    ...
    v_log := v_log || jsonb_build_array(jsonb_build_object('step','verify','detail', jsonb_build_object(
      'return', (SELECT jsonb_build_object('number',r.return_number,'status',r.status,
                    'total',r.total,'credit_note_id',r.vendor_credit_note_id)
      ...
      'credit_note', (SELECT jsonb_build_object('number',c.credit_note_number,'total',c.total,'status',c.status)
                        FROM public.vendor_credit_notes c
                        JOIN public.purchase_returns r ON r.vendor_credit_note_id=c.id
                       WHERE r.id=v_pr)
    )));
