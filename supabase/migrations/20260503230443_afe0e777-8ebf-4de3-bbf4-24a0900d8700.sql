-- Phase 12 — Bank Feeds: extend get_bank_transactions_paginated with branch scoping
...
BEGIN
  ...
  WHERE bt.organization_id = _org_id
    ...
    AND (_branch_id IS NULL OR bt.branch_id = _branch_id OR bt.branch_id IS NULL);
...
GRANT EXECUTE ON FUNCTION public.get_bank_transactions_paginated(
  uuid, uuid, uuid, boolean, text, text, date, date, int, int, uuid
) TO authenticated;

COMMENT ON FUNCTION public.get_bank_transactions_paginated(
  uuid, uuid, uuid, boolean, text, text, date, date, int, int, uuid
) IS
  'Phase 12 — Bank Feeds. Returns paginated bank transactions for the given org/business and optional branch. When _branch_id is supplied, returns rows for that branch + company-wide (NULL) rows. RLS still enforces who can see what.';