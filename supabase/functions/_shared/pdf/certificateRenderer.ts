...
/**
 * certificateRenderer — dedicated PDF renderer for statutory certificates
 * (KE P9, Certificate of Service, and every future equivalent that ships
 * through the localization-pack pipeline).
 *
 * Why not `generateReportPdf`?
 *   Reports are spreadsheets with a title. Certificates are legal
 *   documents with named visual bands: employer identity, employee
 *   identity, fiscal period, monthly breakdown, YTD totals, relief
 *   summary, statutory footnote, signature block. Flattening those
 *   bands into a generic `columns[]/rows[]/summaryRows[]` triad
 *   (the report shape) collapses the identity + signature sections
 *   into a tiny bottom label strip — the shallow output the user
 *   flagged. This renderer draws each ADR-0060 section as its own
 *   labelled band with its own vertical rhythm, so the published
 *   certificate matches what a statutory auditor expects.
 *
 * Country-agnostic: every layout decision comes from `body.sections[]`
 * as authored by the pack publisher. No `if (country === 'KE')`.
 *
...
export async function renderCertificatePdf(
  ...
  const employer = {
    ...
    tax_pin: (opts.branding as any)?.tax_pin ?? payload.employer.tax_pin ?? "",
    address: (opts.branding as any)?.address ?? payload.employer.address ?? "",
    tax_office: (opts.branding as any)?.tax_office ?? payload.employer.tax_office ?? "",
    phone: (opts.branding as any)?.phone ?? payload.employer.phone ?? "",
    email: (opts.branding as any)?.email ?? payload.employer.email ?? "",
  };
  ...
  for (const raw of sections) {
    ...
    switch (type) {
      case "employer_header":
        drawEmployerSection(ctx, raw, employer);
        break;
      case "employee_header":
        drawEmployeeSection(ctx, raw);
        break;
      case "fiscal_period_band":
        drawFiscalPeriodSection(ctx, raw);
        break;
      case "monthly_breakdown":
        drawMonthlyBreakdown(ctx, raw);
        break;
      case "ytd_table":
        drawYtdTable(ctx, raw);
        break;
      case "totals":
        drawTotalsSection(ctx, raw);
        break;
      case "relief_summary":
        drawReliefSection(ctx, raw);
        break;
      case "statutory_footnote":
        drawFootnote(ctx, raw);
        break;
      case "signature_block":
        drawSignatureBlock(ctx, raw);
        break;
      default:
        // certificate_template_v2 schema rejects unknown types at save,
        // but tolerate here for forward compatibility.
        break;
    }
  }
...
function drawMasthead(ctx: DrawCtx, employer: any): void {
  ...
  // Document title, big and centered.
  ...
  // Legal citation, muted italic under subtitle.
...
// ── Section label bar (shared across all bands) ────────────────────
...
// ── Identity bands (employer / employee) ───────────────────────────
...
// ── Monthly breakdown table (12 rows + total) ──────────────────────
...
function drawMonthlyBreakdown(ctx: DrawCtx, spec: CertificateSectionSpec): void {
  ...
  if (cols.length === 0) {
    drawEmpty(ctx, "No monthly columns defined.");
    return;
  }
  ...
  cols.forEach((c, i) => {
    ...
    if (lw > maxLw) {
      sizeLbl = Math.max(6, SIZE.tableHeader * (maxLw / lw));
    }
    const finalW = ctx.fontBold.widthOfTextAtSize(drawLabel, sizeLbl);
    ctx.page.drawText(drawLabel, {
  ...
  const rowH = 14;
  const totals: Record<string, number> = Object.fromEntries(cols.map((c) => [c.key, 0]));
  for (let m = 1; m <= 12; m++) {
...
function drawYtdTable(ctx: DrawCtx, spec: CertificateSectionSpec): void {
  ...
  if (rows.length === 0) {
    drawEmpty(ctx, "No year-to-date entries.");
    return;
  }
  const cols = [
